table 50064 "Store Issue Line"
{
    ...
    begin
        ...
        JobSetup.GET;
        //to    IF JobSetup."Same part usage period@" = TRUE THEN BEGIN
        ...
        IF ("Gen. Prod. Posting Group" = 'TYRE') AND (NOT "Allow Approved Usage") THEN BEGIN
            ...
            //to      ServiceLine.SETRANGE("Allow Approved Usage", FALSE);
            //to      JLE.SETRANGE(JLE."Service Item No.", "Vehicle Registration No.");
            //JLE.SETRANGE(JLE."Service Item No.","Shortcut Dimension 4 Code"); //"Service Item No.");
            //to      JLE.SETFILTER(JLE."Posting Date", '%1..%2', JobSetup."Period Start for Items", JobSetup."Period End for Items");
            ...
            IF JLE.FIND('-') THEN BEGIN
                // REPEAT
                ItemRec.GET("Item No.");
                IF (JQty >= 8) AND (NOT "Allow Approved Usage") THEN
                        ///OR (Quantity > 8)   THEN//(TRUEJQty > 8) THEN
                        BEGIN
                    TESTFIELD("Approve/Reject", 0);
                    "BLocking Notification" := TRUE; //DDADA  100417
                    "Last Inv Date" := JLE."Posting Date";
                    "Quantity CONSM Per Year" := JQty;
                    //to     MESSAGE('TYRE %4 UNITs have been used on this same Vehicle No. %2 between %5 To %6. .\You can only issue 8 TYRES in a Year  .\Further approval is needed to use the part, please contact your Head Of Operations'
                    //to          , "Item No.", "Vehicle Registration No.", JLE."Posting Date", JQty, JobSetup."Period Start for Items", JobSetup."Period End for Items");
                    //to    VALIDATE(Quantity, 0);
                    //to   "Reason For Approval" := 3;

                END
                ELSE BEGIN
                    "Quantity CONSM Per Year" := JQty;
                    //to      MESSAGE('NOTE: \%1 No of Tyre(s) have being used From %2 TO %3 Already', JQty, JobSetup."Period Start for Items", JobSetup."Period End for Items");
                    //to      "Reason For Approval" := 0;
                END;
                //  UNTIL JLE.NEXT =0;
            END;
        ...
        IF ("Gen. Prod. Posting Group" = 'BATTERY') AND (NOT "Allow Approved Usage") THEN BEGIN
            ...
            IF JLE.FIND('-') THEN BEGIN
                // REPEAT
                ItemRec.GET("Item No.");
                IF (JQty >= 2) AND (NOT "Allow Approved Usage") THEN
                        ///OR (Quantity > 8)   THEN//(TRUEJQty > 8) THEN
                        BEGIN
                    TESTFIELD("Approve/Reject", 0);
                    "BLocking Notification" := TRUE; //DDADA  100417
                    "Last Inv Date" := JLE."Posting Date";
                    "Quantity CONSM Per Year" := JQty;
                    //to      MESSAGE('BATTERY(S) %4 UNITs have been used on this same Vehicle No. %2 between %5 To %6. .\You can only issue 2 BATTERIES in a Year  .\Further approval is needed to use the part, please contact your Head Of Operations'
                    //to       , "Item No.", "Vehicle Registration No.", JLE."Posting Date", JQty, JobSetup."Period Start for Items", JobSetup."Period End for Items");
                    VALIDATE(Quantity, 0);
                ...
                ELSE BEGIN
                    "Quantity CONSM Per Year" := JQty;
                    //to       MESSAGE('NOTE: \%1 No of BATTERY(s) have being used From %2 TO %3 Already', JQty, JobSetup."Period Start for Items", JobSetup."Period End for Items");
