codeunit 54530 "HR Dates No Permission Months"
{
    trigger OnRun()
    begin
    end;
    ...
    [Scope('Cloud')]
    procedure DetermineAge(DateOfBirth: Date; DateOfJoin: Date)AgeString: Decimal var
        dayB: Integer;
        monthB: Integer;
        yearB: Integer;
        dayJ: Integer;
        ...
        if((DateOfBirth <> 0D) and (DateOfJoin <> 0D))then begin
            ...
            2, 3, 7: begin
                if(monthJ <> monthB)then begin
                    if monthJ >= monthB then Month:=monthJ - monthB //  ELSE ERROR('The wrong date category!');
                end;
    ...
    procedure DateCategory(BDay: Integer; EDay: Integer; BMonth: Integer; EMonth: Integer; BYear: Integer; EYear: Integer)Category: Integer begin
        if((EYear > BYear) and (EMonth <> BMonth) and (EDay <> BDay))then Category:=1
                                    ...
                                    else if((EYear < BYear))then Error(TEXTDATE1)
                                        else
                                        begin
                                            Category:=0;
    ...
    begin
        ...
        repeat if(NewStartDate > AbsenceHoliday."Start Date") and (NewStartDate < AbsenceHoliday."End Date")then Reserved:=true
            else if(NewEndDate < AbsenceHoliday."End Date") and (NewEndDate > AbsenceHoliday."Start Date")then Reserved:=true
                else if(NewStartDate > AbsenceHoliday."Start Date") and (NewEndDate < AbsenceHoliday."End Date")then Reserved:=true
                    else
                        Reserved:=false;
    ...
    begin
        ...
        while(DateStart <= DateEnd)do begin
            ...
            repeat if(DateStart = StatutoryHoliday."Non Working Dates")then Holiday:=Holiday + StatutoryHoliday.Code;
    ...
    [Scope('Cloud')]
    procedure DetermineAgeYears(DateOfBirth: Date; DateOfJoin: Date)AgeString: Decimal var
        dayB: Integer;
        monthB: Integer;
        yearB: Integer;
        dayJ: Integer;
