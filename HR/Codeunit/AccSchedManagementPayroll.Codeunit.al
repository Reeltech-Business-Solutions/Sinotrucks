codeunit 54402 AccSchedManagementPayroll
{
    ...
    // begin
    //     CheckTemplateAndSetFilter(CurrentSchedName, AccSchedLine);
    //     if AccSchedLine.IsEmpty then exit;
    //     GeneralLedgerSetup.Get;
    //     if CurrentSchedName in[GeneralLedgerSetup."Acc. Sched. for Balance Sheet", GeneralLedgerSetup."Acc. Sched. for Cash Flow Stmt", GeneralLedgerSetup."Acc. Sched. for Income Stmt.", GeneralLedgerSetup."Acc. Sched. for Retained Earn."]then if ConfirmManagement.GetResponse(SystemGeneratedAccSchedQst, true)then begin
    ...
    // end;
    // [Scope('Cloud')]
    // procedure LookupName(CurrentSchedName: Code[10]; var EntrdSchedName: Text[10]): Boolean var
    //     AccSchedName: Record "Acc. Schedule Name";
    // begin
    //     AccSchedName.Name:=CurrentSchedName;
    //     if PAGE.RunModal(0, AccSchedName) <> ACTION::LookupOK then exit(false);
    ...
    //     if ColumnLayout.Find('-')then repeat TempColumnLayout:=ColumnLayout;
    //             TempColumnLayout.Insert;
    //         until ColumnLayout.Next = 0;
    //     if TempColumnLayout.Find('-')then;
    // end;
    // [Scope('Cloud')]
    // procedure LookupColumnName(CurrentColumnName: Code[10]; var EntrdColumnName: Text[10]): Boolean var
    //     ColumnLayoutName: Record "Column Layout Name";
    // begin
    //     ColumnLayoutName.Name:=CurrentColumnName;
    //     if PAGE.RunModal(0, ColumnLayoutName) <> ACTION::LookupOK then exit(false);
    ...
    //             AccSchedName.Get(CurrentSchedName);
    //         end;
    //         if TestColumnName then if CurrentColumnName <> ColumnLayoutName.Name then begin
    //                 CheckColumnTemplateName(CurrentColumnName);
    //                 ColumnLayoutName.Get(CurrentColumnName);
    //             end;
    ...
    //             if ColumnLayout2.Find('-')then repeat AnyColumnDimensions:=(ColumnLayout2."Dimension 1 Totaling" <> '') or (ColumnLayout2."Dimension 2 Totaling" <> '') or (ColumnLayout2."Dimension 3 Totaling" <> '') or (ColumnLayout2."Dimension 4 Totaling" <> '');
    //                 until AnyColumnDimensions or (ColumnLayout2.Next = 0);
    //             if AnyColumnDimensions then Error(Text024, AccSchedName.FieldCaption("Analysis View Name"), AccSchedName.TableCaption, AccSchedName."Analysis View Name", ColumnLayoutName.FieldCaption("Analysis View Name"), ColumnLayoutName.TableCaption, ColumnLayoutName."Analysis View Name");
    //         end;
    //     end;
    // end;
    ...
    //     RangeToInt: Integer;
    // begin
    //     if ColumnLayout."Comparison Period Formula" = '' then exit;
    //     ColumnLayout.ParsePeriodFormula(ColumnLayout."Comparison Period Formula", Steps, Type, RangeFromType, RangeToType, RangeFromInt, RangeToInt);
    //     AccountingPeriodMgt.AccPeriodStartEnd(Date, StartDate, EndDate, PeriodError, Steps, Type, RangeFromType, RangeToType, RangeFromInt, RangeToInt);
    // end;
    ...
    // end;
    // [Scope('Cloud')]
    // procedure CalcCell(var AccSchedLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout"; CalcAddCurr: Boolean): Decimal var
    //     Result: Decimal;
    // begin
    ...
    //     if(OldAccSchedLineFilters <> AccSchedLine.GetFilters) or (OldColumnLayoutFilters <> ColumnLayout.GetFilters) or (OldAccSchedLineName <> AccSchedLine."Schedule Name") or (OldColumnLayoutName <> ColumnLayout."Column Layout Name") or (OldCalcAddCurr <> CalcAddCurr) or Recalculate then begin
    ...
    //         case Show of Show::"When Positive": if Result < 0 then Result:=0;
    //         Show::"When Negative": if Result > 0 then Result:=0;
    //         end;
    //         if "Show Opposite Sign" then Result:=-Result;
    //         case "Show Indented Lines" of "Show Indented Lines"::"Indented Only": if AccSchedLine.Indentation = 0 then Result:=0;
    //         "Show Indented Lines"::"Non-Indented Only": if AccSchedLine.Indentation > 0 then Result:=0;
    //         end;
    //     end;
    //     if AccSchedLine."Show Opposite Sign" then Result:=-Result;
    //     OnBeforeCalcCellExit(AccSchedLine, ColumnLayout, CalcAddCurr, Result);
    //     exit(Result);
    // end;
    // [Scope('Cloud')]
    // procedure CalcCellValue(AccSchedLine: Record "Acc. Schedule Line"; ColumnLayout: Record "Column Layout"; CalcAddCurr: Boolean): Decimal var
    ...
    // begin
    //     Result:=0;
    //     if AccSchedLine.Totaling <> '' then if AccSchedCellValue.Get(AccSchedLine."Line No.", ColumnLayout."Line No.")then begin
    ...
    //         else
    //         begin
    //             if ColumnLayout."Column Type" = ColumnLayout."Column Type"::Formula then Result:=EvaluateExpression(false, ColumnLayout.Formula, AccSchedLine, ColumnLayout, CalcAddCurr)
    //             else if AccSchedLine."Totaling Type" in[AccSchedLine."Totaling Type"::Formula, AccSchedLine."Totaling Type"::"Set Base For Percent"]then Result:=EvaluateExpression(true, AccSchedLine.Totaling, AccSchedLine, ColumnLayout, CalcAddCurr)
    //                 else if(StartDate = 0D) or (EndDate = 0D) or (EndDate = DMY2Date(31, 12, 9999))then begin
    //                         Result:=0;
    //                         PeriodError:=true;
    //                     end
    //                     else
    //                         case AccSchedLine."Totaling Type" of AccSchedLine."Totaling Type"::"Posting Accounts", AccSchedLine."Totaling Type"::"Total Accounts": begin
    ...
    //                             if(AccSchedLine."Totaling Type" = AccSchedLine."Totaling Type"::"Posting Accounts") and (StrLen(AccSchedLine.Totaling) <= MaxStrLen(GLAcc.Totaling)) and (StrPos(AccSchedLine.Totaling, '*') = 0)then begin
    ...
    //                             end
    //                             else if GLAcc.Find('-')then repeat Result:=Result + CalcGLAcc(GLAcc, AccSchedLine, ColumnLayout, CalcAddCurr);
    //                                     until GLAcc.Next = 0;
    //                         end;
    //                         AccSchedLine."Totaling Type"::"Cost Type", AccSchedLine."Totaling Type"::"Cost Type Total": begin
    //                             AccSchedLine.CopyFilters(AccountScheduleLine);
    //                             SetCostTypeRowFilters(CostType, AccSchedLine, ColumnLayout);
    //                             SetCostTypeColumnFilters(CostType, AccSchedLine, ColumnLayout);
    //                             if(AccSchedLine."Totaling Type" = AccSchedLine."Totaling Type"::"Cost Type") and (StrLen(AccSchedLine.Totaling) <= MaxStrLen(GLAcc.Totaling)) and (StrPos(AccSchedLine.Totaling, '*') = 0)then begin
    ...
    //                             end
    //                             else if CostType.Find('-')then repeat Result:=Result + CalcCostType(CostType, AccSchedLine, ColumnLayout, CalcAddCurr);
    //                                     until CostType.Next = 0;
    //                         end;
    //                         AccSchedLine."Totaling Type"::"Cash Flow Entry Accounts", AccSchedLine."Totaling Type"::"Cash Flow Total Accounts": begin
    //                             AccSchedLine.CopyFilters(AccountScheduleLine);
    //                             SetCFAccRowFilter(CFAccount, AccSchedLine);
    //                             SetCFAccColumnFilter(CFAccount, AccSchedLine, ColumnLayout);
    //                             if(AccSchedLine."Totaling Type" = AccSchedLine."Totaling Type"::"Cash Flow Entry Accounts") and (StrLen(AccSchedLine.Totaling) <= 30)then begin
    ...
    //                             end
    //                             else if CFAccount.Find('-')then repeat Result:=Result + CalcCFAccount(CFAccount, AccSchedLine, ColumnLayout);
    ...
    //     exit(Result);
    // end;
    // local procedure CalcGLAcc(var GLAcc: Record "G/L Account"; var AccSchedLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout"; CalcAddCurr: Boolean)ColValue: Decimal var
    ...
    //     if IsHandled then exit(ColValue);
    //     UseDimFilter:=false;
    //     if AccSchedName.Name <> AccSchedLine."Schedule Name" then AccSchedName.Get(AccSchedLine."Schedule Name");
    //     if ConflictAmountType(AccSchedLine, ColumnLayout."Amount Type", AmountType)then exit(0);
    ...
    //         case ColumnLayout."Ledger Entry Type" of ColumnLayout."Ledger Entry Type"::Entries: if AccSchedName."Analysis View Name" = '' then with GLEntry do begin
    //                     SetGLAccGLEntryFilters(GLAcc, GLEntry, AccSchedLine, ColumnLayout, UseBusUnitFilter, UseDimFilter);
    //                     case AmountType of AmountType::"Net Amount": begin
    ...
    //                         Balance:=ColValue;
    //                     end;
    //                     AmountType::"Debit Amount": if CalcAddCurr then begin
    ...
    //                         end;
    //                     AmountType::"Credit Amount": if CalcAddCurr then begin
    //                             if TestBalance then begin
    ...
    //                     end;
    //                 end;
    //         ColumnLayout."Ledger Entry Type"::"Budget Entries": begin
    //             if AccSchedName."Analysis View Name" = '' then with GLBudgEntry do begin
    //                     SetGLAccGLBudgetEntryFilters(GLAcc, GLBudgEntry, AccSchedLine, ColumnLayout, UseBusUnitFilter, UseDimFilter);
    //                     case AmountType of AmountType::"Net Amount": begin
    //                         CalcSums(Amount);
    //                         ColValue:=Amount;
    //                     end;
    //                     AmountType::"Debit Amount": begin
    //                         CalcSums(Amount);
    //                         ColValue:=Amount;
    //                         if ColValue < 0 then ColValue:=0;
    //                     end;
    //                     AmountType::"Credit Amount": begin
    //                         CalcSums(Amount);
    //                         ColValue:=-Amount;
    //                         if ColValue < 0 then ColValue:=0;
    //                     end;
    ...
    //                     Balance:=Amount;
    //                 end;
    //             if CalcAddCurr then ColValue:=CalcLCYToACY(ColValue);
    //         end;
    //         end;
    //         OnBeforeTestBalance(GLAcc, AccSchedName, AccSchedLine, ColumnLayout, AmountType, ColValue, CalcAddCurr, TestBalance, GLEntry, GLBudgEntry);
    //         if TestBalance then begin
    //             if AccSchedLine.Show = AccSchedLine.Show::"When Positive Balance" then if Balance < 0 then exit(0);
    //             if AccSchedLine.Show = AccSchedLine.Show::"When Negative Balance" then if Balance > 0 then exit(0);
    //         end;
    //     end;
    //     exit(ColValue);
    // end;
    // local procedure CalcCFAccount(var CFAccount: Record "Cash Flow Account"; var AccSchedLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout")ColValue: Decimal var
    ...
    //     if AccSchedName.Name <> AccSchedLine."Schedule Name" then AccSchedName.Get(AccSchedLine."Schedule Name");
    //     if ConflictAmountType(AccSchedLine, ColumnLayout."Amount Type", AmountType)then exit(0);
    //     if ColumnLayout."Column Type" <> ColumnLayout."Column Type"::Formula then case ColumnLayout."Ledger Entry Type" of ColumnLayout."Ledger Entry Type"::Entries: if AccSchedName."Analysis View Name" = '' then with CFForecastEntry do begin
    //                     SetCFEntryFilters(CFAccount, CFForecastEntry, AccSchedLine, ColumnLayout);
    //                     case ColumnLayout."Amount Type" of ColumnLayout."Amount Type"::"Net Amount": begin
    //                         CalcSums("Amount (LCY)");
    //                         ColValue:="Amount (LCY)";
    //                     end;
    //                     end;
    ...
    // begin
    //     with AccSchedLine2 do case "Totaling Type" of "Totaling Type"::"Posting Accounts": begin
    //             GLAcc.SetFilter("No.", Totaling);
    //             GLAcc.SetRange("Account Type", GLAcc."Account Type"::Posting);
    //         end;
    //         "Totaling Type"::"Total Accounts": begin
    //             GLAcc.SetFilter("No.", Totaling);
    ...
    //         if UseBusUnitFilter then if UseDimFilter then SetCurrentKey("G/L Account No.", "Business Unit Code", "Global Dimension 1 Code", "Global Dimension 2 Code")
    //             else
    //                 SetCurrentKey("G/L Account No.", "Business Unit Code", "Posting Date")
    //         else if UseDimFilter then SetCurrentKey("G/L Account No.", "Global Dimension 1 Code", "Global Dimension 2 Code")
    //             else
    //                 SetCurrentKey("G/L Account No.", "Posting Date");
    //         if GLAcc.Totaling = '' then SetRange("G/L Account No.", GLAcc."No.")
    //         else
    ...
    //         if UseBusUnitFilter or UseDimFilter then SetCurrentKey("Budget Name", "G/L Account No.", "Business Unit Code", "Global Dimension 1 Code", "Global Dimension 2 Code", "Budget Dimension 1 Code", "Budget Dimension 2 Code", "Budget Dimension 3 Code", "Budget Dimension 4 Code", Date)
    //         else
    ...
    //         FilterGroup(2);
    //         SetDimFilters(GetDimTotalingFilter(1, AccSchedLine."Dimension 1 Totaling"), GetDimTotalingFilter(2, AccSchedLine."Dimension 2 Totaling"), GetDimTotalingFilter(3, AccSchedLine."Dimension 3 Totaling"), GetDimTotalingFilter(4, AccSchedLine."Dimension 4 Totaling"));
    //         FilterGroup(8);
    //         SetDimFilters(GetDimTotalingFilter(1, ColumnLayout."Dimension 1 Totaling"), GetDimTotalingFilter(2, ColumnLayout."Dimension 2 Totaling"), GetDimTotalingFilter(3, ColumnLayout."Dimension 3 Totaling"), GetDimTotalingFilter(4, ColumnLayout."Dimension 4 Totaling"));
    //         SetFilter("Business Unit Code", ColumnLayout."Business Unit Totaling");
    //         FilterGroup(0);
    //     end;
    ...
    //         if GLAcc.Totaling = '' then SetRange("Account No.", GLAcc."No.")
    //         else
    ...
    //         case "Totaling Type" of "Totaling Type"::"Cash Flow Entry Accounts": begin
    //             CFAccount.SetFilter("No.", Totaling);
    //             CFAccount.SetRange("Account Type", CFAccount."Account Type"::Entry);
    //         end;
    //         "Totaling Type"::"Cash Flow Total Accounts": begin
    //             CFAccount.SetFilter("No.", Totaling);
    ...
    //         case "Column Type" of "Column Type"::"Net Change": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": GLAcc.SetRange("Date Filter", FromDate, ToDate);
    //             AccSchedLine2."Row Type"::"Beginning Balance": GLAcc.SetFilter("Date Filter", '..%1', ClosingDate(FromDate - 1)); // always includes closing date
    //             AccSchedLine2."Row Type"::"Balance at Date": GLAcc.SetRange("Date Filter", 0D, ToDate);
    //             end;
    //         "Column Type"::"Balance at Date": if AccSchedLine2."Row Type" = AccSchedLine2."Row Type"::"Beginning Balance" then GLAcc.SetRange("Date Filter", 0D) // Force a zero return
    //             else
    //                 GLAcc.SetRange("Date Filter", 0D, ToDate);
    //         "Column Type"::"Beginning Balance": if AccSchedLine2."Row Type" = AccSchedLine2."Row Type"::"Balance at Date" then GLAcc.SetRange("Date Filter", 0D) // Force a zero return
    //             else
    //                 GLAcc.SetRange("Date Filter", 0D, ClosingDate(FromDate - 1));
    //         "Column Type"::"Year to Date": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": GLAcc.SetRange("Date Filter", FiscalStartDate2, ToDate);
    //             AccSchedLine2."Row Type"::"Beginning Balance": GLAcc.SetFilter("Date Filter", '..%1', ClosingDate(FiscalStartDate2 - 1)); // always includes closing date
    //             AccSchedLine2."Row Type"::"Balance at Date": GLAcc.SetRange("Date Filter", 0D, ToDate);
    //             end;
    //         "Column Type"::"Rest of Fiscal Year": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": GLAcc.SetRange("Date Filter", CalcDate('<+1D>', ToDate), AccountingPeriodMgt.FindEndOfFiscalYear(FiscalStartDate2));
    //             AccSchedLine2."Row Type"::"Beginning Balance": GLAcc.SetRange("Date Filter", 0D, ToDate);
    //             AccSchedLine2."Row Type"::"Balance at Date": GLAcc.SetRange("Date Filter", 0D, AccountingPeriodMgt.FindEndOfFiscalYear(ToDate));
    //             end;
    //         "Column Type"::"Entire Fiscal Year": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": GLAcc.SetRange("Date Filter", FiscalStartDate2, AccountingPeriodMgt.FindEndOfFiscalYear(FiscalStartDate2));
    //             AccSchedLine2."Row Type"::"Beginning Balance": GLAcc.SetFilter("Date Filter", '..%1', ClosingDate(FiscalStartDate2 - 1)); // always includes closing date
    //             AccSchedLine2."Row Type"::"Balance at Date": GLAcc.SetRange("Date Filter", 0D, AccountingPeriodMgt.FindEndOfFiscalYear(ToDate));
    //             end;
    //         end;
    //     end;
    //     OnAfterSetGLAccColumnFilters(GLAcc, AccSchedLine2, ColumnLayout)end;
    // [Scope('Cloud')]
    ...
    //         case "Column Type" of "Column Type"::"Net Change": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CFAccount.SetRange("Date Filter", FromDate, ToDate);
    //             AccSchedLine2."Row Type"::"Beginning Balance": CFAccount.SetFilter("Date Filter", '..%1', ClosingDate(FromDate - 1));
    //             AccSchedLine2."Row Type"::"Balance at Date": CFAccount.SetRange("Date Filter", 0D, ToDate);
    //             end;
    //         "Column Type"::"Balance at Date": if AccSchedLine2."Row Type" = AccSchedLine2."Row Type"::"Beginning Balance" then CFAccount.SetRange("Date Filter", 0D) // Force a zero return
    //             else
    //                 CFAccount.SetRange("Date Filter", 0D, ToDate);
    //         "Column Type"::"Beginning Balance": if AccSchedLine2."Row Type" = AccSchedLine2."Row Type"::"Balance at Date" then CFAccount.SetRange("Date Filter", 0D) // Force a zero return
    //             else
    //                 CFAccount.SetRange("Date Filter", 0D, CalcDate('<-1D>', FromDate));
    //         "Column Type"::"Year to Date": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CFAccount.SetRange("Date Filter", FiscalStartDate2, ToDate);
    //             AccSchedLine2."Row Type"::"Beginning Balance": CFAccount.SetFilter("Date Filter", '..%1', FiscalStartDate2 - 1);
    //             AccSchedLine2."Row Type"::"Balance at Date": CFAccount.SetRange("Date Filter", 0D, ToDate);
    //             end;
    //         "Column Type"::"Rest of Fiscal Year": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CFAccount.SetRange("Date Filter", CalcDate('<+1D>', ToDate), AccountingPeriodMgt.FindEndOfFiscalYear(FiscalStartDate2));
    //             AccSchedLine2."Row Type"::"Beginning Balance": CFAccount.SetRange("Date Filter", 0D, ToDate);
    //             AccSchedLine2."Row Type"::"Balance at Date": CFAccount.SetRange("Date Filter", 0D, AccountingPeriodMgt.FindEndOfFiscalYear(ToDate));
    //             end;
    //         "Column Type"::"Entire Fiscal Year": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CFAccount.SetRange("Date Filter", FiscalStartDate2, AccountingPeriodMgt.FindEndOfFiscalYear(FiscalStartDate2));
    //             AccSchedLine2."Row Type"::"Beginning Balance": CFAccount.SetFilter("Date Filter", '..%1', ClosingDate(FiscalStartDate2 - 1));
    //             AccSchedLine2."Row Type"::"Balance at Date": CFAccount.SetRange("Date Filter", 0D, AccountingPeriodMgt.FindEndOfFiscalYear(ToDate));
    //             end;
    //         end;
    //     end;
    ...
    //         SetCurrentKey("Cash Flow Account No.", "Cash Flow Forecast No.", "Global Dimension 1 Code", "Global Dimension 2 Code", "Cash Flow Date");
    //         if CFAccount.Totaling = '' then SetRange("Cash Flow Account No.", CFAccount."No.")
    //         else
    ...
    //         if CFAccount.Totaling = '' then SetRange("Account No.", CFAccount."No.")
    //         else
    ...
    //                         if AccSchedLine.Find('-')then repeat if AccSchedLine."Line No." <> AccSchedLineID then Result:=Result + CalcCellValue(AccSchedLine, ColumnLayout, CalcAddCurr);
    ...
    //                         if ColumnLayout.Find('-')then repeat if ColumnLayout."Line No." <> AccSchedLineID then Result:=Result + CalcCellValue(AccSchedLine, ColumnLayout, CalcAddCurr);
    //                             until ColumnLayout.Next = 0
    ...
    // end;
    // [Scope('Cloud')]
    // procedure FormatCellAsText(var ColumnLayout2: Record "Column Layout"; Value: Decimal; CalcAddCurr: Boolean): Text[30]var
    //     ValueAsText: Text[30];
    // begin
    //     //ValueAsText := MatrixMgt.FormatValue(Value, ColumnLayout2."Rounding Factor", CalcAddCurr);
    //     if(ValueAsText <> '') and (ColumnLayout2."Column Type" = ColumnLayout2."Column Type"::Formula) and (StrPos(ColumnLayout2.Formula, '%') > 1)then ValueAsText:=ValueAsText + '%';
    //     exit(ValueAsText);
    // end;
    // [Scope('Cloud')]
    // procedure GetDivisionError(): Boolean begin
    //     exit(DivisionError);
    // end;
    // [Scope('Cloud')]
    // procedure GetPeriodError(): Boolean begin
    //     exit(PeriodError);
    // end;
    ...
    //             if GLAcc.FindSet then repeat AccSchedLine.Init;
    ...
    //                     end;
    //                     if GLAcc."Account Type" in[GLAcc."Account Type"::Total, GLAcc."Account Type"::"End-Total"]then AccSchedLine."Totaling Type":=AccSchedLine."Totaling Type"::"Total Accounts"
    //                     else
    ...
    //             AccSchedLineNo:=AccSchedLine."Line No.";
    //             MoveAccSchedLines(AccSchedLine, AccCounter);
    //             if CashFlowAcc.FindSet then repeat AccSchedLine.Init;
    //                     AccSchedLineNo:=AccSchedLineNo + 10000;
    //                     AccSchedLine."Line No.":=AccSchedLineNo;
    //                     AccSchedLine.Description:=CashFlowAcc.Name;
    //                     if CashFlowAcc."Account Type" in[CashFlowAcc."Account Type"::Entry, CashFlowAcc."Account Type"::Total, CashFlowAcc."Account Type"::"End-Total"]then begin
    //                         AccSchedLine.Totaling:=CashFlowAcc."No.";
    //                         AccSchedLine."Row No.":=CopyStr(CashFlowAcc."No.", 1, MaxStrLen(AccSchedLine."Row No."));
    //                     end;
    //                     if CashFlowAcc."Account Type" in[CashFlowAcc."Account Type"::Total, CashFlowAcc."Account Type"::"End-Total"]then AccSchedLine."Totaling Type":=AccSchedLine."Totaling Type"::"Cash Flow Total Accounts"
    //                     else
    ...
    //             AccSchedLineNo:=AccSchedLine."Line No.";
    //             MoveAccSchedLines(AccSchedLine, AccCounter);
    //             if CostType.FindSet then repeat AccSchedLine.Init;
    //                     AccSchedLineNo:=AccSchedLineNo + 10000;
    //                     AccSchedLine."Line No.":=AccSchedLineNo;
    //                     AccSchedLine.Description:=CostType.Name;
    //                     if CostType.Type in[CostType.Type::"Cost Type", CostType.Type::Total, CostType.Type::"End-Total"]then begin
    //                         AccSchedLine.Totaling:=CostType."No.";
    //                         AccSchedLine."Row No.":=CopyStr(CostType."No.", 1, MaxStrLen(AccSchedLine."Row No."));
    //                     end;
    //                     if CostType.Type in[CostType.Type::Total, CostType.Type::"End-Total"]then AccSchedLine."Totaling Type":=AccSchedLine."Totaling Type"::"Cost Type Total"
    //                     else
    ...
    //     end;
    // end;
    // local procedure ExchangeAmtAddCurrToLCY(AmountLCY: Decimal): Decimal begin
    //     if not GLSetupRead then begin
    //         GLSetup.Get;
    //         GLSetupRead:=true;
    //     end;
    //     exit(CurrExchRate.ExchangeAmtLCYToFCY(WorkDate, GLSetup."Additional Reporting Currency", AmountLCY, CurrExchRate.ExchangeRate(WorkDate, GLSetup."Additional Reporting Currency")));
    // end;
    // [Scope('Cloud')]
    ...
    // end;
    // [Scope('Cloud')]
    // procedure GetDimTotalingFilter(DimNo: Integer; DimTotaling: Text[250]): Text[1024]var
    ...
    //     i: Integer;
    // begin
    //     if DimTotaling = '' then exit(DimTotaling);
    //     DimTotaling2:=DimTotaling;
    //     repeat i:=StrPos(DimTotaling2, '|');
    //         if i > 0 then begin
    //             DimTotalPart:=CopyStr(DimTotaling2, 1, i - 1);
    //             if i < StrLen(DimTotaling2)then DimTotaling2:=CopyStr(DimTotaling2, i + 1)
    //             else
    //                 DimTotaling2:='';
    //         end
    //         else
    ...
    //         if ResultFilter2 <> '' then if StrLen(ResultFilter) + StrLen(ResultFilter2) + 1 > MaxStrLen(ResultFilter)then Error(Text021, DimTotaling);
    //         if ResultFilter <> '' then ResultFilter:=ResultFilter + '|';
    //         ResultFilter:=CopyStr(ResultFilter + ResultFilter2, 1, MaxStrLen(ResultFilter));
    //     until i <= 0;
    //     exit(ResultFilter);
    // end;
    // local procedure ConvDimTotalingFilter(DimNo: Integer; DimTotaling: Text[250]): Text[1024]var
    ...
    //     case DimNo of 1: DimCode:=AnalysisView."Dimension 1 Code";
    //     2: DimCode:=AnalysisView."Dimension 2 Code";
    //     3: DimCode:=AnalysisView."Dimension 3 Code";
    //     4: DimCode:=AnalysisView."Dimension 4 Code";
    //     5: DimCode:=CostAccSetup."Cost Center Dimension";
    //     6: DimCode:=CostAccSetup."Cost Object Dimension";
    //     end;
    //     if DimCode = '' then exit(DimTotaling);
    //     DimVal.SetRange("Dimension Code", DimCode);
    //     DimVal.SetFilter(Code, DimTotaling);
    //     if DimVal.Find('-')then repeat DimValTotaling:=DimVal."Dimension Value Type" in[DimVal."Dimension Value Type"::Total, DimVal."Dimension Value Type"::"End-Total"];
    //             if DimValTotaling and (DimVal.Totaling <> '')then begin
    //                 if StrLen(ResultFilter) + StrLen(DimVal.Totaling) + 1 > MaxStrLen(ResultFilter)then Error(Text021, DimTotaling);
    //                 if ResultFilter <> '' then ResultFilter:=ResultFilter + '|';
    //                 ResultFilter:=ResultFilter + DimVal.Totaling;
    //             end;
    //         until(DimVal.Next = 0) or not DimValTotaling;
    //     if DimValTotaling then exit(ResultFilter);
    //     exit(DimTotaling);
    // end;
    // local procedure CalcCostType(var CostType: Record "Cost Type"; var AccSchedLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout"; CalcAddCurr: Boolean)ColValue: Decimal var
    ...
    //         case ColumnLayout."Ledger Entry Type" of ColumnLayout."Ledger Entry Type"::Entries: begin
    //             with CostEntry do begin
    //                 if UseDimFilter then SetCurrentKey("Cost Type No.", "Cost Center Code", "Cost Object Code")
    //                 else
    //                     SetCurrentKey("Cost Type No.", "Posting Date");
    //                 if CostType.Totaling = '' then SetRange("Cost Type No.", CostType."No.")
    //                 else
    ...
    //                     if TestBalance then Balance:=CostEntry."Additional-Currency Amount";
    ...
    //             case AmountType of AmountType::"Net Amount": ColValue:=CostBudgEntry.Amount;
    //             AmountType::"Debit Amount": if CostBudgEntry.Amount > 0 then ColValue:=CostBudgEntry.Amount;
    //             AmountType::"Credit Amount": if CostBudgEntry.Amount < 0 then ColValue:=CostBudgEntry.Amount;
    //             end;
    ...
    // begin
    //     with AccSchedLine2 do begin
    //         case "Totaling Type" of "Totaling Type"::"Cost Type": begin
    //             CostType.SetFilter("No.", Totaling);
    //             CostType.SetRange(Type, CostType.Type::"Cost Type");
    //         end;
    //         "Totaling Type"::"Cost Type Total": begin
    //             CostType.SetFilter("No.", Totaling);
    ...
    //         if ColumnLayout."Ledger Entry Type" = ColumnLayout."Ledger Entry Type"::"Budget Entries" then CostType.SetFilter("Budget Filter", GetFilter("Cost Budget Filter"));
    //     end;
    // end;
    // [Scope('Cloud')]
    ...
    //     with ColumnLayout do begin
    //         CalcColumnDates(ColumnLayout, FromDate, ToDate, FiscalStartDate2);
    //         case "Column Type" of "Column Type"::"Net Change": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CostType.SetRange("Date Filter", FromDate, ToDate);
    //             AccSchedLine2."Row Type"::"Beginning Balance": CostType.SetFilter("Date Filter", '<%1', FromDate);
    //             AccSchedLine2."Row Type"::"Balance at Date": CostType.SetRange("Date Filter", 0D, ToDate);
    //             end;
    //         "Column Type"::"Balance at Date": if AccSchedLine2."Row Type" = AccSchedLine2."Row Type"::"Beginning Balance" then CostType.SetRange("Date Filter", 0D) // Force a zero return
    //             else
    //                 CostType.SetRange("Date Filter", 0D, ToDate);
    //         "Column Type"::"Beginning Balance": if AccSchedLine2."Row Type" = AccSchedLine2."Row Type"::"Balance at Date" then CostType.SetRange("Date Filter", 0D) // Force a zero return
    //             else
    //                 CostType.SetRange("Date Filter", 0D, CalcDate('<-1D>', FromDate));
    //         "Column Type"::"Year to Date": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CostType.SetRange("Date Filter", FiscalStartDate2, ToDate);
    //             AccSchedLine2."Row Type"::"Beginning Balance": CostType.SetFilter("Date Filter", '<%1', FiscalStartDate2);
    //             AccSchedLine2."Row Type"::"Balance at Date": CostType.SetRange("Date Filter", 0D, ToDate);
    //             end;
    //         "Column Type"::"Rest of Fiscal Year": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CostType.SetRange("Date Filter", CalcDate('<+1D>', ToDate), AccountingPeriodMgt.FindEndOfFiscalYear(FiscalStartDate2));
    //             AccSchedLine2."Row Type"::"Beginning Balance": CostType.SetRange("Date Filter", 0D, ToDate);
    //             AccSchedLine2."Row Type"::"Balance at Date": CostType.SetRange("Date Filter", 0D, AccountingPeriodMgt.FindEndOfFiscalYear(ToDate));
    //             end;
    //         "Column Type"::"Entire Fiscal Year": case AccSchedLine2."Row Type" of AccSchedLine2."Row Type"::"Net Change": CostType.SetRange("Date Filter", FiscalStartDate2, AccountingPeriodMgt.FindEndOfFiscalYear(FiscalStartDate2));
    //             AccSchedLine2."Row Type"::"Beginning Balance": CostType.SetFilter("Date Filter", '<%1', FiscalStartDate2);
    //             AccSchedLine2."Row Type"::"Balance at Date": CostType.SetRange("Date Filter", 0D, AccountingPeriodMgt.FindEndOfFiscalYear(ToDate));
    //             end;
    //         end;
    ...
    // end;
    // [Scope('Cloud')]
    // procedure HasDimFilter(var AccSchedLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout"): Boolean begin
    //     exit((AccSchedLine."Dimension 1 Totaling" <> '') or (AccSchedLine."Dimension 2 Totaling" <> '') or (AccSchedLine."Dimension 3 Totaling" <> '') or (AccSchedLine."Dimension 4 Totaling" <> '') or (AccSchedLine.GetFilter("Dimension 1 Filter") <> '') or (AccSchedLine.GetFilter("Dimension 2 Filter") <> '') or (AccSchedLine.GetFilter("Dimension 3 Filter") <> '') or (AccSchedLine.GetFilter("Dimension 4 Filter") <> '') or (ColumnLayout."Dimension 1 Totaling" <> '') or (ColumnLayout."Dimension 2 Totaling" <> '') or (ColumnLayout."Dimension 3 Totaling" <> '') or (ColumnLayout."Dimension 4 Totaling" <> '') or (ColumnLayout."Cost Center Totaling" <> '') or (ColumnLayout."Cost Object Totaling" <> ''));
    // end;
    // local procedure HasCostDimFilter(var AccSchedLine: Record "Acc. Schedule Line"): Boolean begin
    //     exit((AccSchedLine."Cost Center Totaling" <> '') or (AccSchedLine."Cost Object Totaling" <> '') or (AccSchedLine.GetFilter("Cost Center Filter") <> '') or (AccSchedLine.GetFilter("Cost Object Filter") <> ''));
    // end;
    // [Scope('Cloud')]
    ...
    //         if(StartDate = CalcDate('<-CM>', StartDate)) and (FromDate = CalcDate('<-CM>', FromDate)) and (EndDate = CalcDate('<CM>', EndDate))then ToDate:=CalcDate('<CM>', ToDate);
    //         FiscalStartDate2:=AccountingPeriodMgt.FindFiscalYear(ToDate);
    //     end
    //     else if ComparisonPeriodFormula <> '' then begin
    ...
    //     AccSchedLineNo:=AccSchedLine."Line No.";
    //     AccSchedLine.SetRange("Schedule Name", AccSchedLine."Schedule Name");
    //     if AccSchedLine.Find('+')then repeat I:=AccSchedLine."Line No.";
    ...
    //     EndDate:=NewEndDate;
    // end;
    // local procedure ConflictAmountType(AccSchedLine: Record "Acc. Schedule Line"; ColumnLayoutAmtType: Option "Net Amount", "Debit Amount", "Credit Amount"; var AmountType: Option): Boolean begin
    //     if(ColumnLayoutAmtType = AccSchedLine."Amount Type") or (AccSchedLine."Amount Type" = AccSchedLine."Amount Type"::"Net Amount")then AmountType:=ColumnLayoutAmtType
    //     else if ColumnLayoutAmtType = ColumnLayoutAmtType::"Net Amount" then AmountType:=AccSchedLine."Amount Type"
    //         else
    //             exit(true);
    //     exit(false);
    // end;
    ...
    //             if ErrorType <> ErrorType::None then Message(StrSubstNo(ColumnFormulaErrorMsg, TempColumnLayout.Formula, Format(ErrorType)))
    //             else
    ...
    //             exit;
    //         end;
    //         if Totaling = '' then exit;
    //         if "Totaling Type" in["Totaling Type"::"Cash Flow Entry Accounts", "Totaling Type"::"Cash Flow Total Accounts"]then DrillDownOnCFAccount(TempColumnLayout, AccScheduleLine)
    //         else
    ...
    //     with AccScheduleLine do begin
    //         if "Totaling Type" in["Totaling Type"::Formula, "Totaling Type"::"Set Base For Percent"]then Message(RowFormulaMsg, Totaling)
    //         else
    ...
    // begin
    //     with AccScheduleLine do if "Totaling Type" in["Totaling Type"::"Cost Type", "Totaling Type"::"Cost Type Total"]then begin
    ...
    //                 GLAcc.FilterGroup(0);
    //                 PAGE.Run(PAGE::"Chart of Accounts (G/L)", GLAcc)end
    //             else
    //             begin
    ...
    //                 GLAccAnalysisView.SetDimFilters(GetDimTotalingFilter(1, "Dimension 1 Totaling"), GetDimTotalingFilter(2, "Dimension 2 Totaling"), GetDimTotalingFilter(3, "Dimension 3 Totaling"), GetDimTotalingFilter(4, "Dimension 4 Totaling"));
    //                 GLAccAnalysisView.FilterGroup(8);
    //                 GLAccAnalysisView.SetDimFilters(GetDimTotalingFilter(1, TempColumnLayout."Dimension 1 Totaling"), GetDimTotalingFilter(2, TempColumnLayout."Dimension 2 Totaling"), GetDimTotalingFilter(3, TempColumnLayout."Dimension 3 Totaling"), GetDimTotalingFilter(4, TempColumnLayout."Dimension 4 Totaling"));
    ...
    //             PAGE.Run(PAGE::"Chart of Cash Flow Accounts", CFAccount)end
    //         else
    //         begin
    ...
    //         if GetRangeMin("Date Filter") = GetRangeMax("Date Filter")then SetRange("Date Filter", GetRangeMin("Date Filter"));
    //     end;
    // end;
    // [Scope('Cloud')]
    ...
    //     if AccSchedCellValue.FindFirst then case true of AccSchedCellValue."Has Error": ErrorType:=ErrorType::"Division by Zero";
    //         AccSchedCellValue."Period Error": ErrorType:=ErrorType::"Period Error";
    //         AccSchedCellValue."Has Error" and AccSchedCellValue."Period Error": ErrorType:=ErrorType::Both;
    //         end;
    ...
    // end;
    // local procedure CalcLCYToACY(ColValue: Decimal): Decimal begin
    //     if not GLSetupRead then begin
    //         GLSetup.Get;
    //         GLSetupRead:=true;
    //         if GLSetup."Additional Reporting Currency" <> '' then AddRepCurrency.Get(GLSetup."Additional Reporting Currency");
    //     end;
    //     if GLSetup."Additional Reporting Currency" <> '' then exit(Round(ExchangeAmtAddCurrToLCY(ColValue), AddRepCurrency."Amount Rounding Precision"));
    //     exit(0);
    // end;
