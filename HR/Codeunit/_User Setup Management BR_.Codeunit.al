codeunit 54523 "User Setup Management BR"
{
    ...
    begin
        ...
        if (AccRespCenter <> '') and (UserRespCenter <> '') and (AccRespCenter <> UserRespCenter) then Message(Text002 + Text003, AccType, RespCenter.TableCaption, AccRespCenter, UserRespCenter);
    ...
    begin
        ...
        if (UserRespCenter <> '') and (AccRespCenter <> UserRespCenter) then
            exit(false)
        else
            exit(true);
    end;
    ...
    begin
        ...
        if RespCenter.Get(RespCenterCode) then if RespCenter."Location Code" <> '' then UserLocation := RespCenter."Location Code";
        if AccLocation <> '' then
            exit(AccLocation)
        else
            exit(UserLocation);
    ...
    begin
        ...
        case DimensionNo of
            1:
                begin
                    if (UserSetup.Get(UserCode)) and (UserCode <> '') then if UserSetup."Shortcut Dimension 1 Code" <> '' then DimensionVal := UserSetup."Shortcut Dimension 1 Code";
                    exit(DimensionVal);
                end;
            2:
                begin
                    if (UserSetup.Get(UserCode)) and (UserCode <> '') then if UserSetup."Shortcut Dimension 2 Code" <> '' then DimensionVal := UserSetup."Shortcut Dimension 2 Code";
                    exit(DimensionVal);
                end;
            3:
                begin
                    if (UserSetup.Get(UserCode)) and (UserCode <> '') then if UserSetup."Shortcut Dimension 3 Code" <> '' then DimensionVal := UserSetup."Shortcut Dimension 3 Code";
                    exit(DimensionVal);
                end;
            4:
                begin
                    if (UserSetup.Get(UserCode)) and (UserCode <> '') then if UserSetup."Shortcut Dimension 4 Code" <> '' then DimensionVal := UserSetup."Shortcut Dimension 4 Code";
                    exit(DimensionVal);
                end;
        end;
    end;
