codeunit 54531 prPayrollProcessing
{
    // ++Note
    ...
    begin
        ...
        */
        ...
        if curBasicPay > 0 then begin
            ...
            TSubGroupOrder := 1;
            NoOfUnits := 0;
            fnUpdatePeriodTrans(strEmpCode, 'BPAY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, Membership, ReferenceNo, SelectedPeriod, Dept, salariesAcc, JournalPostAs::Debit, JournalPostingType::"G/L Account", '', CoopParameters::none);
            //Salary Arrears
            ...
            if prSalaryArrears.Find('-') then begin
                repeat
                    ...
                    TSubGroupOrder := 2;
                    NoOfUnits := 0;
                    fnUpdatePeriodTrans(strEmpCode, prSalaryArrears."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, Membership, ReferenceNo, SelectedPeriod, Dept, salariesAcc, JournalPostAs::Debit, JournalPostingType::"G/L Account", '', CoopParameters::none);
                    ...
                    TSubGroupOrder := 4;
                    NoOfUnits := 0;
                    fnUpdatePeriodTrans(strEmpCode, 'PYAR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, Membership, ReferenceNo, SelectedPeriod, Dept, TaxAccount, JournalPostAs::Debit, JournalPostingType::"G/L Account", '', CoopParameters::none) until prSalaryArrears.Next = 0;
            end;
            //Get Earnings
            currAnnualPay := 0;
            curNonTaxable := 0; //Deji RBS 02/09/2024
            prEmployeeTransactions.Reset;
            prEmployeeTransactions.SetCurrentKey("Employee Code", "Transaction Code", "Period Month", "Period Year", Suspended);
            ...
            if prEmployeeTransactions.Find('-') then begin
                ...
                repeat
                    ...
                    if prTransactionCodes.Find('-') then begin
                        ...
                        //PKK Added
                        if prTransactionCodes."Excl. from Proration" = true then curTransAmount := 0;
                        //PKK Added
                        //PKK Prorate absence
                        if ProrateAbsence = true then begin
                            if prTransactionCodes."Prorate Absence" = true then begin
                                if curTransAmount > 0 then begin
                                    if DayAbsent > 0 then begin
                                        ...
                                        DaysWorked := fnDaysWorked(SelectedPeriod, false);
                                        if ProrateAbsMonthDays = true then
                                            curTransAmount := fnBasicPayProrated(strEmpCode, intMonth, intYear, curTransAmount, CountDaysofMonth - DayAbsent, CountDaysofMonth)
                                        else
                        ...
                        if (not prTransactionCodes.Taxable) and (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::Ignore) then curNonTaxable := curNonTaxable + curTransAmount;
                        //Added to ensure special transaction that are not taxable are not inlcuded in list of Allowances
                        if (not prTransactionCodes.Taxable) and (prTransactionCodes."Special Transactions" <> prTransactionCodes."Special Transactions"::Ignore) then curTransAmount := 0;
                        if prTransactionCodes."Leave Allowance" then //urTransAmount:=CheckProrateLeave(strEmpCode,prTransactionCodes."Transaction Code",dtOpenPeriod);
                            Ann_Leave := curTransAmount;
                        //Annual Leaves
                        //PKK - Annual Trans
                        if prTransactionCodes."Annual Pay" = true then currAnnualPay := currAnnualPay + curTransAmount;
            ...
            curGrossPay := 0;   //Ify 22/05/2024 
            GrossExArr := 0;
            GrossExArr := 0;
            curGrossPay := (curBasicPay + curTotAllowances + curSalaryArrears); //+curNonTaxable; //RBS
            GrossExArr := (curBasicPay + curTotAllowances);
            curTransAmount := curGrossPay;
            strTransDescription := 'Gross Pay';
            TGroup := 'GROSS PAY';
            TGroupOrder := 4;
            TSubGroupOrder := 0;
            fnUpdatePeriodTrans(strEmpCode, 'GPAY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::"G/L Account", '', CoopParameters::none);
            ...
            if prEmployeeTransactions.Find('-') then begin
                ...
                repeat
                    ...
                    if prTransactionCodes.Find('-') then begin
                        ...
                        fnUpdatePeriodTrans(strEmpCode, prTransactionCodes."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curPassage, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                    end;
                until prEmployeeTransactions.Next = 0;
            end;
            //PKK - NG Relief
            ...
            */
            ...
            /* //Annual Relief
             ...
             IF (((((curGrossPay-(currAnnualPay+curNonTaxable))*12)+currAnnualPay) * 0.01) +
                ...
                (curReliefPersonal+((curReliefGrossPerc/100)*(curGro ssPay*12))) THEN
            ...
            */
            ...
            NHFAmt_G := 0;//Lateef RBS 04082023
            ...
            if intNHIF_BasedOn = intNHIF_BasedOn::"Taxable Pay" then curNhif_Base_Amount := curTaxablePay;
            ...
            if blnPaysNhif then begin
                ...
                TGroupOrder := 7;
                TSubGroupOrder := 2;
                fnUpdatePeriodTrans(strEmpCode, 'NHF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, NHIFEMPyee, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::none);
            end;
            ...
            if (curReliefChild + curReliefDependant) > 0
                then
                MonthlyReliefAmount := MonthlyReliefAmount + ((curReliefChild + curReliefDependant));
            ///12 //PKK-NG 
  //PKK
            //PKK NG
            curReliefPersonal := 0;
            ...
            //Get the NSSF amount
            if blnPaysNssf then curNSSF := curNssfEmployee;
            curTransAmount := curNSSF;
            strTransDescription := 'N.S.S.F';
            TGroup := 'STATUTORIES';
            TGroupOrder := 7;
            TSubGroupOrder := 1;
            fnUpdatePeriodTrans(strEmpCode, 'NSSF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, NSSFEMPyee, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::NSSF);
            ...
            TSubGroupOrder := 1;
            NoOfUnits := 0;
            fnUpdatePeriodTrans(strEmpCode, 'DEFCON', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
            ...
            if blnGetsPAYERelief then begin
                ...
                TGroupOrder := 6;
                TSubGroupOrder := 9;
                fnUpdatePeriodTrans(strEmpCode, 'PSNR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
            end
            else
                curReliefPersonal := 0;
            //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            //>Pension Contribution [self] relief
            curPensionStaff := fnGetSpecialTransAmount(strEmpCode, intMonth, intYear, SpecialTransType::"Defined Contribution", false); //Self contrib Pension is 1 on [Special Transaction]
            if curPensionStaff > 0 then begin
                ...
                TGroupOrder := 6;
                TSubGroupOrder := 2;
                fnUpdatePeriodTrans(strEmpCode, 'PNSR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none)
            end;
            ...
            if blnPaysPaye and blnGetsPAYERelief then begin
                ...
                if curInsuranceReliefAmount > 0 then begin
                    ...
                    TGroupOrder := 6;
                    TSubGroupOrder := 8;
                    fnUpdatePeriodTrans(strEmpCode, 'INSR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                end;
                //>OOI
                ...
                if curOOI > 0 then begin
                    ...
                    TGroupOrder := 6;
                    TSubGroupOrder := 3;
                    fnUpdatePeriodTrans(strEmpCode, 'OOI', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                end;
                //HOSP
                ...
                if curHOSP > 0 then begin
                    ...
                    TGroupOrder := 6;
                    TSubGroupOrder := 4;
                    fnUpdatePeriodTrans(strEmpCode, 'HOSP', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                end;
                ...
                if curNonTaxable > 0 then begin
                    ...
                    TGroupOrder := 6;
                    TSubGroupOrder := 5;
                    fnUpdatePeriodTrans(strEmpCode, 'NONTAX', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curNonTaxable, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                end;
            end;
            //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            /*
            ...
            TGroupOrder := 8;
            TSubGroupOrder := 7;
            fnUpdatePeriodTrans(strEmpCode, 'MonthRF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
            //RBS-BANJI
            ...
            if (curPensionStaff + curDefinedContrib) > curMaxPensionContrib then //curTaxablePay:= curGrossTaxable - (curSalaryArrears + curMaxPensionContrib+curOOI+curHOSP+curNonTaxable+NonTaxable) //RBS
            ...
            TGroupOrder := 6;
            TSubGroupOrder := 6;
            fnUpdatePeriodTrans(strEmpCode, 'TXBP', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
            /*
            VitalSetupS.RESET;
            ...
            */
            ...
            curTaxablePay2 := curTaxablePay; //RBS 02/09/24
            ...
            //if curTaxCharged <> 0 then curTaxCharged := curTaxCharged / 12;
            if curTaxCharged <> 0 then curTaxCharged := curTaxCharged;
            ...
            */
            //PKK - Relief before tax computation
            //PKK-NG ADDED
            if (curGrossPay * 0.01) > curTaxCharged then curTaxCharged := (curGrossPay * 0.01);
            //PKK-NG ADDED
            //PKK- NG
            ...
            TGroupOrder := 6;
            TSubGroupOrder := 7;
            fnUpdatePeriodTrans(strEmpCode, 'TXCHRG', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
            ...
            TGroupOrder := 7;
            TSubGroupOrder := 3;
            fnUpdatePeriodTrans(strEmpCode, 'PAYE', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, TaxAccount, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::none);
            ...
            if prSalaryCard.Get(strEmpCode) then begin
                if prSalaryCard."Pays PAYE" then begin
                    ...
                    if PeriodTrans.Find('-') then begin
                        repeat
                            if PeriodTrans."Period Year" <> 2015 then PayeTillDate += PeriodTrans.Amount;
                    ...
                    TGroupOrder := 7;
                    TSubGroupOrder := 5;
                    fnUpdatePeriodTrans(strEmpCode, 'TOT-PAYE', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                end;
            end;
            ...
            if curPAYE < 0 then begin
                ...
                if prUnusedRelief.Find('-') then prUnusedRelief.Delete;
            ...
            if prEmployeeTransactions.Find('-') then begin
                ...
                repeat
                    ...
                    if prTransactionCodes.Find('-') then begin
                        ...
                        if (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::Morgage) and (prTransactionCodes."Deduct Mortgage" = false) then begin
                            curTransAmount := 0;
                        end;
                        //Get the posting Details
                        JournalPostingType := JournalPostingType::" ";
                        ...
                        if (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::"Staff Loan") and (prTransactionCodes."Repayment Method" = prTransactionCodes."Repayment Method"::Amortized) then begin
                            curTransAmount := 0;
                            curLoanInt := 0;
                            curLoanInt := fnCalcLoanInterest(strEmpCode, prEmployeeTransactions."Transaction Code", prTransactionCodes."Nominal Interest Rate", prTransactionCodes."Repayment Method", prEmployeeTransactions."Original Amount", prEmployeeTransactions.Balance, SelectedPeriod, false);
                            //Post the Interest
                            if (curLoanInt <> 0) then begin
                                ...
                                TSubGroupOrder := 1;
                                NoOfUnits := prEmployeeTransactions."No of Units";
                                fnUpdatePeriodTrans(strEmpCode, strTransCode, TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, Dept, JournalAcc, JournalPostAs::Credit, JournalPostingType, prEmployeeTransactions."Loan Number", CoopParameters::"loan Interest")
                            end;
 ...
 begin
                        ...
                        fnUpdatePeriodTrans(strEmpCode, prEmployeeTransactions."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, Dept, JournalAcc, JournalPostAs::Credit, JournalPostingType, prEmployeeTransactions."Loan Number", prTransactionCodes."coop parameters");
                        //Check if transaction is loan. Get the Interest on the loan & post it at this point before moving next ****Loan Calculation
                        if (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::"Staff Loan") and (prTransactionCodes."Repayment Method" <> prTransactionCodes."Repayment Method"::Amortized) then begin
                            curLoanInt := fnCalcLoanInterest(strEmpCode, prEmployeeTransactions."Transaction Code", prTransactionCodes."Nominal Interest Rate", prTransactionCodes."Repayment Method", prEmployeeTransactions."Original Amount", prEmployeeTransactions.Balance, SelectedPeriod, prTransactionCodes.Welfare);
                            if curLoanInt > 0 then begin
                                curTransAmount := curLoanInt;
                                curTotalDeductions := curTotalDeductions + curTransAmount; //Sum-up all the deductions
                                curTransBalance := 0;
                        ...
                        if prTransactionCodes."Fringe Benefit" = true then begin
                            if prTransactionCodes."Nominal Interest Rate" < curLoanMarketRate then begin
                                fnCalcFringeBenefit := (((curLoanMarketRate - prTransactionCodes."Nominal Interest Rate") * curLoanCorpRate) / 1200) * prEmployeeTransactions.Balance;
                            end;
                        end
                        else begin
                        ...
                        if fnCalcFringeBenefit > 0 then begin
                            fnUpdateEmployerDeductions(strEmpCode, prEmployeeTransactions."Transaction Code" + '-FRG', 'EMP', TGroupOrder, TSubGroupOrder, 'Fringe Benefit Tax', fnCalcFringeBenefit, 0, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod)
                        end;
                        //End Fringe Benefits
                        ...
                        if (prTransactionCodes."Employer Deduction") or (prTransactionCodes."Include Employer Deduction") then begin
                            ...
                            if curTransAmount > 0 then fnUpdateEmployerDeductions(strEmpCode, prEmployeeTransactions."Transaction Code", 'EMP', TGroupOrder, TSubGroupOrder, '', curTransAmount, 0, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod);
                            fnUpdatePeriodTrans(strEmpCode, 'PENSNORM', TGroup, TGroupOrder, TSubGroupOrder, 'Pension ER', curTransAmount, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, Dept, '', 0, JournalPostingType, prEmployeeTransactions."Loan Number", prTransactionCodes."coop parameters");
                            fnUpdatePeriodTrans(strEmpCode, 'PENSNOR', TGroup, TGroupOrder, TSubGroupOrder, 'Pension EE', EERelief, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, Dept, '', 0, JournalPostingType, prEmployeeTransactions."Loan Number", prTransactionCodes."coop parameters");
                        end;
                        //Employer deductions
                    end;
                until prEmployeeTransactions.Next = 0;
                ...
                TSubGroupOrder := 9;
                NoOfUnits := prEmployeeTransactions."No of Units";
                fnUpdatePeriodTrans(strEmpCode, strTransCode, TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, TotDed, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                EmployerAmount := 0; //PKK
                EmployerBalance := 0; //PKK
            ...
            TSubGroupOrder := 0;
            NoOfUnits := 0;
            fnUpdatePeriodTrans(strEmpCode, 'NPAY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, PayablesAcc, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::none);
    ...
    begin
        ...
        */
        //W days in month
        MonthDays := CalcDate('-1D', CalcDate('1M', DMY2Date(1, Date2DMY(TodayDate, 2), Date2DMY(TodayDate, 3)))) - DMY2Date(1, Date2DMY(TodayDate, 2), Date2DMY(TodayDate, 3));
        ...
        repeat
            ...
            if VitalSetup."Prol. Based on days in month" = true then begin
                ...
                if (Date2DWY(TDate, 1) <> 6) and (Date2DWY(TDate, 1) <> 7) then DaysInMonth := DaysInMonth + 1;
            end;
        ...
        with prPeriodTransactions do begin
            ...
            if TraCode.Get(TCode) then
                if TraCode.Pension then begin
                    ...
                    if EmpTra.FindFirst then Amount := Round(curAmount + EmpTra.Amount, 0.01, '=');
                end;
            if Trans.Get(TCode) then begin
                if Trans."Non-Transactional" = false then begin
                    if Trans.Frequency = Trans.Frequency::Varied then prPeriodTransactions."Payslip Order" := 2
                end;
            end;
            if Trans.Get(TCode) then begin
                if Trans.Pension then begin
    ...
    var
        ...
        // KeepCount: Integer;
        // ReducingAmount: decimal;
        // Reduction: decimal;   tolu 14/8/24
    ...
    begin
        ...
        // ReducingAmount := 0;       tolu 14/8/24
        // ReducingAmount := curTaxable2Payee;
        // PAYE := 0;
        // KeepCount := 0;
        // prPAYE.reset;
        // if prPAYE.FindFirst() then
        //     repeat
        //         Reduction := 0;
        //         KeepCount += 1;
        //         if KeepCount <> prPAYE.Count then begin //it is not the final loop
        //             if ReducingAmount > prPAYE."PAYE Tier" then
        //                 PAYE += (prPAYE."PAYE Tier" * (prPAYE.Rate / 100))
        //             else
        //                 PAYE += (ReducingAmount * (prPAYE.Rate / 100)); //when the amount is less than the PAYE tier

        //             if ReducingAmount > prPAYE."PAYE Tier" then begin
        //                 Reduction := ReducingAmount - prPAYE."PAYE Tier";
        //                 ReducingAmount := Reduction;
        //             end else
        //                 exit(PAYE);
        //         end else //it is the final leg
        //         begin
        //             PAYE += (ReducingAmount * (prPAYE.Rate / 100)); //charge tax on the remaining amount
        //             exit(PAYE);
        //         end;
        //     until prPAYE.Next() = 0;  tolu 14/8/24
        ...
        //                 if KeepCount = prPAYE.Count then //this is the last record or loop
        //  curTaxable2Payee:=curTempAmount
        //                 else if curTempAmount >= prPAYE."PAYE Tier" then curTempAmount:=prPAYE."PAYE Tier"
        //                     else
        //                         curTempAmount:=curTempAmount;
        //                 PAYE:=PAYE + (curTempAmount * (prPAYE.Rate / 100));
        //                 curTaxable2Payee:=curTaxable2Payee - curTempAmount;
    ...
    begin
        ...
        if prNHIF.FindFirst then begin
            repeat
                if ((curBaseAmount >= prNHIF."Lower Limit") and (curBaseAmount <= prNHIF."Upper Limit")) then NHIF := prNHIF.Amount;
            until prNHIF.Next = 0;
        end;
    end;
    ...
    begin
        ...
        if prPayrollPeriods.Find('+') then NewQuarter := CalcDate('1Q', prPayrollPeriods."Date Opened");
        ...
        prEmployeeTransactions.SetRange(prEmployeeTransactions."Period Year", intYear);
        //Multiple Payroll
        if ControlInfo."Multiple Payroll" then prEmployeeTransactions.SetRange(prEmployeeTransactions."Payroll Code", PayrollCode);
        if prEmployeeTransactions.Find('-') then begin
            repeat
                prTransactionCodes.Reset;
                prTransactionCodes.SetRange(prTransactionCodes."Transaction Code", prEmployeeTransactions."Transaction Code");
                ...
                if (prTransactionCodes.Frequency = prTransactionCodes.Frequency::Fixed) and (prEmployeeTransactions."Stop for Next Period" = false) then //DENNO ADDED THIS TO CHECK FREQUENCY AND STOP IF MARKED
 begin
 ...
 begin
                        if ((prTransactionCodes."Balance Type" = prTransactionCodes."Balance Type"::Reducing) and (curTransBalance <> 0)) or (prTransactionCodes."Balance Type" <> prTransactionCodes."Balance Type"::Reducing) then prEmployeeTransactions.Balance := curTransBalance;
                        prEmployeeTransactions.Modify;
        ...
        if ControlInfo."Multiple Payroll" then prPayrollPeriods.SetRange(prPayrollPeriods."Payroll Code", PayrollCode);
        ...
        with prNewPayrollPeriods do begin
            ...
            Closed := false;
            "Payroll Code" := PayrollCode;
            if dtNewPeriod = NewQuarter then Quarter := true;
            Insert;
        end;
    ...
    begin
        ...
        if prEmployee.Find('-') then begin
            repeat
                ...
                if P9NetPay <> 0 then fnUpdateP9Table(prEmployee."No.", P9BasicPay, P9Allowances, P9Benefits, P9ValueOfQuarters, P9DefinedContribution, P9OwnerOccupierInterest, P9GrossPay, P9TaxablePay, P9TaxCharged, P9InsuranceRelief, P9TaxRelief, P9Paye, P9NSSF, P9NHIF, P9Deductions, P9NetPay, dtCurPeriod, PayrollCode);
    ...
    begin
        ...
        if PayTillCutOff = true then begin //PKK - AGROSACK
            ...
            RemainingDays := (CalcDate('-1D', CalcDate('1M', DMY2Date(1, Date2DMY(TodayDate, 2), Date2DMY(TodayDate, 3)))) - TodayDate);
        end;
        TDate := TodayDate;
        i := 0;
        repeat
            ...
            if VitalSetup."Prol. Based on days in month" = true then begin
                ...
                if (Date2DWY(TDate, 1) <> 6) and (Date2DWY(TDate, 1) <> 7) then DaysWorked := DaysWorked + 1;
            end;
            TDate := CalcDate('1D', TDate);
 ...
 begin
                                ...
                                if (Date2DMY(dtDOE, 2) = Date2DMY(StartDate, 2)) and (Date2DMY(dtDOE, 3) = Date2DMY(StartDate, 3)) then begin
                                    CountDaysofMonth := fnDaysInMonth(dtDOE);
                                    DaysWorked := fnDaysWorked(dtDOE, false);
                                    if DontProrateBPAY = false then ProratedBasic := fnBasicPayProrated(EmpCode, startmonth, startYear, P9BasicPay, DaysWorked, CountDaysofMonth)
                                end;
                                //PKK - Prorate Absence
                                //PKK - Prorate Absence
                                //Prorate Basic Pay on    {What if someone leaves within the same month they are employed}
            ...
            if SalaryArrears <> 0 then begin
                ...
                fnUpdateSalaryArrears(EmpCode, TransCode, StartDate, EndDate, SalaryArrears, PAYEArrears, PeriodMonth, PeriodYear, dtOpenPeriod);
            end
        end
        else
        ...
        if Employee.Get(EmpCode) then begin
            if (Date2DMY(Employee."Date Of Join", 2) = Month) and (Date2DMY(Employee."Date Of Join", 3) = Year) then begin
                if Trans.Get(TransCode) then begin
                    if Trans."Is Formula" = false then exit;
                end;
            end;
        end;
        if ProrateAbsence = true then begin
    ...
    begin
        ...
        else if ((GrossPay * 12) * 0.01) < TaxRelief_Value then begin
            T2hOr1PercentGP := TaxRelief_Value;
        end;
        T2hOr1PercentGP_Permonth := (T2hOr1PercentGP / 12);
                ...
                if not SalCard."Suspend Pay" then begin
                    ...
                    */
                    ...
                    if curBasicPay > 0 then begin
                        ...
                        TSubGroupOrder := 1;
                        NoOfUnits := 0;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'BPAY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", salariesAcc, JournalPostAs::Debit, JournalPostingType::"G/L Account", '', CoopParameters::none);
                        //Salary Arrears
                        prSalaryArrears.Reset;
                        ...
                        if prSalaryArrears.Find('-') then begin
                            repeat
                                ...
                                TSubGroupOrder := 2;
                                NoOfUnits := 0;
                                fnUpdatePeriodTrans(EmployeeRec."No.", prSalaryArrears."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", salariesAcc, JournalPostAs::Debit, JournalPostingType::"G/L Account", '', CoopParameters::none);
                                //Insert [PAYE Arrears] into period trans [PYAR]
                                curTransAmount := curPayeArrears;
                                strTransDescription := 'P.A.Y.E Arrears';
                                TGroup := 'STATUTORIES';
                                ...
                                TSubGroupOrder := 4;
                                NoOfUnits := 0;
                                fnUpdatePeriodTrans(EmployeeRec."No.", 'PYAR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", TaxAccount, JournalPostAs::Debit, JournalPostingType::"G/L Account", '', CoopParameters::none) until prSalaryArrears.Next = 0;
                        end;
                        //Get Earnings
                        currAnnualPay := 0;
                        ...
                        if prEmployeeTransactions.Find('-') then begin
                            ...
                            repeat
                                ...
                                if prTransactionCodes.Find('-') then begin
                                        ...
                                        if prTransactionCodes."Prorate Absence" = true then begin
                                            if curTransAmount > 0 then begin
                                                if DayAbsent > 0 then begin
                                                    ...
                                                    DaysWorked := fnDaysWorked(SelectedPeriod, false);
                                                    if ProrateAbsMonthDays = true then
                                                        curTransAmount := fnBasicPayProrated(EmployeeRec."No.", intMonth, intYear, curTransAmount, CountDaysofMonth - DayAbsent, CountDaysofMonth)
                                                    else
                                    ...
                                    //End posting Details
                                    NoOfUnits := prEmployeeTransactions."No of Units";
                                    fnUpdatePeriodTrans(EmployeeRec."No.", prTransactionCodes."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, EmployeeRec."Department Code", JournalAcc, JournalPostAs::Debit, JournalPostingType, '', prTransactionCodes."coop parameters");
                                end;
                            until prEmployeeTransactions.Next = 0;
                        end;
                        //IF SalCard.GET(strEmpCode) THEN
                        ...
                        TGroupOrder := 4;
                        TSubGroupOrder := 0;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'GPAY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::"G/L Account", '', CoopParameters::none);
                        //  curGrossPay := (curBasicPay + curTotAllowances + curSalaryArrears); //RBS
                        curGrossPay := (curBasicPay + curTotAllowances + curSalaryArrears);
                        //George
                        prEmployeeTransactions.Reset;
                        ...
                        if prEmployeeTransactions.Find('-') then begin
                            ...
                            repeat
                                ...
                                if prTransactionCodes.Find('-') then begin
                                    curPassage := curGrossPay * prTransactionCodes."Passage %";
                                    strTransDescription := prPeriodTransactions."Transaction Name";
                                    fnUpdatePeriodTrans(EmployeeRec."No.", prTransactionCodes."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curPassage, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                                end;
                            until prEmployeeTransactions.Next = 0;
                        end;
                        //PKK - NG Relief
                        ...
                        */
                        ...
                        if SalCard."Pays NHF" then begin
                            ...
                            TGroupOrder := 7;
                            TSubGroupOrder := 2;
                            fnUpdatePeriodTrans(EmployeeRec."No.", 'NHF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", NHIFEMPyee, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::none);
                        end;
                        //PKK
                        //MonthlyReliefAmount:=MonthlyReliefAmount+curNHIF;
                        //MonthlyReliefAmount:=MonthlyReliefAmount+(curNHIF*12); //RBS BANJI
                        if (curReliefChild + curReliefDependant) > 0 then MonthlyReliefAmount := MonthlyReliefAmount + ((curReliefChild + curReliefDependant));
                        ///12 //PKK-NG 
  //PKK
                        //PKK NG
                        curReliefPersonal := 0;
                        //PKK - NG Relief
                        //Get the NSSF amount
                        if SalCard."Pays NSSF" then curNSSF := curNssfEmployee;
                        curTransAmount := curNSSF;
                        strTransDescription := 'N.S.S.F';
                        TGroup := 'STATUTORIES';
                        TGroupOrder := 7;
                        TSubGroupOrder := 1;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'NSSF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", NSSFEMPyee, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::NSSF);
                        //Get the Defined contribution to post based on the Max Def contrb allowed   ****************All Defined Contributions not included
                        curDefinedContrib := curNSSF; //(curNSSF + curPensionStaff + curNonTaxable) - curMorgageReliefAmount
                        curTransAmount := curDefinedContrib;
                        strTransDescription := 'Defined Contributions';
                        ...
                        TSubGroupOrder := 1;
                        NoOfUnits := 0;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'DEFCON', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                        //Get the Gross taxable amount
                        //>GrossTaxable = Gross + Benefits + nValueofQuarters  ******Confirm CurValueofQuaters
                        curGrossTaxable := curGrossPay + curBenefits + curValueOfQuarters;
                        //message('%1',curGrossTaxable);
                        ...
                        if blnGetsPAYERelief then begin
                            ...
                            TGroupOrder := 6;
                            TSubGroupOrder := 9;
                            fnUpdatePeriodTrans(EmployeeRec."No.", 'PSNR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                        end
                        else
                            curReliefPersonal := 0;
                        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        //>Pension Contribution [self] relief
                        curPensionStaff := fnGetSpecialTransAmount(EmployeeRec."No.", intMonth, intYear, SpecialTransType::"Defined Contribution", false); //Self contrib Pension is 1 on [Special Transaction]
                        if curPensionStaff > 0 then begin
                            if curPensionStaff > curMaxPensionContrib then
                                curTransAmount := curMaxPensionContrib
                            else
                            ...
                            TGroupOrder := 6;
                            TSubGroupOrder := 2;
                            fnUpdatePeriodTrans(EmployeeRec."No.", 'PNSR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none)
                        end;
                        //if he PAYS paye only*******************I
                        if SalCard."Pays PAYE" and blnGetsPAYERelief then begin
                            //Get Insurance Relief
                            curInsuranceReliefAmount := fnGetSpecialTransAmount(EmployeeRec."No.", intMonth, intYear, SpecialTransType::"Life Insurance", false); //Insurance is 3 on [Special Transaction]
                            if curInsuranceReliefAmount > 0 then begin
                                curTransAmount := curInsuranceReliefAmount;
                                strTransDescription := 'Insurance Relief';
                                TGroup := 'TAX CALCULATIONS';
                                TGroupOrder := 6;
                                TSubGroupOrder := 8;
                                fnUpdatePeriodTrans(EmployeeRec."No.", 'INSR', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                            end;
                            //>OOI
                            curOOI := fnGetSpecialTransAmount(EmployeeRec."No.", intMonth, intYear, SpecialTransType::"Owner Occupier Interest", false); //Morgage is LAST on [Special Transaction]
                            if curOOI > 0 then begin
                                if curOOI <= curOOIMaxMonthlyContrb then
                                    curTransAmount := curOOI
                                else
                                ...
                                TGroupOrder := 6;
                                TSubGroupOrder := 3;
                                fnUpdatePeriodTrans(EmployeeRec."No.", 'OOI', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                            end;
                            //HOSP
                            curHOSP := fnGetSpecialTransAmount(EmployeeRec."No.", intMonth, intYear, SpecialTransType::"Home Ownership Savings Plan", false); //Home Ownership Savings Plan
                            if curHOSP > 0 then begin
                                if curHOSP <= curReliefMorgage then
                                    curTransAmount := curHOSP
                                else
                                ...
                                TGroupOrder := 6;
                                TSubGroupOrder := 4;
                                fnUpdatePeriodTrans(EmployeeRec."No.", 'HOSP', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                            end;
                            //Enter NonTaxable Amount
                            if curNonTaxable > 0 then begin
                                strTransDescription := 'Other Non-Taxable Benefits';
                                ...
                                TGroupOrder := 6;
                                TSubGroupOrder := 5;
                                fnUpdatePeriodTrans(EmployeeRec."No.", 'NONTAX', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curNonTaxable, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                            end;
                        end;
                        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        /*
                        ...
                        TGroupOrder := 8;
                        TSubGroupOrder := 7;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'MonthRF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                        //RBS-BANJI
                        //MESSAGE('%1',MonthlyReliefAmount);
                        //Get the Taxable amount for calculation of PAYE
                        //>prTaxablePay = (GrossTaxable - SalaryArrears) - (TheDefinedToPost + curSelfPensionContrb + MorgageRelief)
                        ...
                        TGroupOrder := 6;
                        TSubGroupOrder := 6;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'TXBP', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                        /*
                        VitalSetupS.RESET;
                        VitalSetupS.SETRANGE(VitalSetupS."Medical Scheme No.",'321456');
                        IF VitalSetupS.COUNT <> 4 THEN BEGIN
                        ...
                        */
                        ...
                        //curTaxablePay:=((curTaxablePay-currAnnualPay)*12)+currAnnualPay;
                        //curTaxablePay2 := ((curTaxablePay - currAnnualPay) * 12) + currAnnualPay; //RBS LATEEF 22072021
                        curTaxablePay2 := curTaxablePay;   //RBS 30-08-24
                        //IF (curTaxablePay-MonthlyReliefAmount) < 300000 THEN //PKK Tax for those earning less than 300000
                        //curTaxCharged := (curTaxablePay-MonthlyReliefAmount) * 0.01
                        //ELSE
                        curTaxCharged := fnGetEmployeePaye(curTaxablePay2); //RBS BANJI -(MonthlyReliefAmount));
                        ...
                        */
                        ...
                        TGroupOrder := 6;
                        TSubGroupOrder := 7;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'TXCHRG', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                        //Get the Net PAYE amount to post for the month
                        if (curReliefPersonal + curInsuranceReliefAmount) > curMaximumRelief then
                            curPAYE := curTaxCharged - curMaximumRelief
                        else
                        ...
                        TGroupOrder := 7;
                        TSubGroupOrder := 3;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'PAYE', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", TaxAccount, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::none);
                        if prSalaryCard.Get(EmployeeRec."No.") then begin
                            if prSalaryCard."Pays PAYE" then begin
                                ...
                                TGroupOrder := 7;
                                TSubGroupOrder := 5;
                                fnUpdatePeriodTrans(EmployeeRec."No.", 'TOT-PAYE', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                            end;
                        end;
                        //Store the unused relief for the current month
                        //>If Paye<0 then "Insert into tblprUNUSEDRELIEF
                            ...
                            repeat
                                ...
                                if prTransactionCodes.Find('-') then begin
                                    ...
                                    if (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::"Staff Loan") and (prTransactionCodes."Repayment Method" = prTransactionCodes."Repayment Method"::Amortized) then begin
                                        curTransAmount := 0;
                                        curLoanInt := 0;
                                        curLoanInt := fnCalcLoanInterest(EmployeeRec."No.", prEmployeeTransactions."Transaction Code", prTransactionCodes."Nominal Interest Rate", prTransactionCodes."Repayment Method", prEmployeeTransactions."Original Amount", prEmployeeTransactions.Balance, SelectedPeriod, false);
                                        //Post the Interest
                                        if (curLoanInt <> 0) then begin
                                            curTransAmount := curLoanInt;
                                            curTotalDeductions := curTotalDeductions + curTransAmount; //Sum-up all the deductions
 ...
 begin
                                    ...
                                    fnUpdatePeriodTrans(EmployeeRec."No.", prEmployeeTransactions."Transaction Code", TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, EmployeeRec."Department Code", JournalAcc, JournalPostAs::Credit, JournalPostingType, prEmployeeTransactions."Loan Number", prTransactionCodes."coop parameters");
                                    //Check if transaction is loan. Get the Interest on the loan & post it at this point before moving next ****Loan Calculation
                                    if (prTransactionCodes."Special Transactions" = prTransactionCodes."Special Transactions"::"Staff Loan") and (prTransactionCodes."Repayment Method" <> prTransactionCodes."Repayment Method"::Amortized) then begin
                                        curLoanInt := fnCalcLoanInterest(EmployeeRec."No.", prEmployeeTransactions."Transaction Code", prTransactionCodes."Nominal Interest Rate", prTransactionCodes."Repayment Method", prEmployeeTransactions."Original Amount", prEmployeeTransactions.Balance, SelectedPeriod, prTransactionCodes.Welfare);
                                        if curLoanInt > 0 then begin
                                            curTransAmount := curLoanInt;
                                            curTotalDeductions := curTotalDeductions + curTransAmount; //Sum-up all the deductions
                                            curTransBalance := 0;
                                    ...
                                    if fnCalcFringeBenefit > 0 then begin
                                        fnUpdateEmployerDeductions(EmployeeRec."No.", prEmployeeTransactions."Transaction Code" + '-FRG', 'EMP', TGroupOrder, TSubGroupOrder, 'Fringe Benefit Tax', fnCalcFringeBenefit, 0, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod)
                                    end;
                                    //End Fringe Benefits
                                    //Create Employer Deduction
                                    EmployerAmount := 0;
                                    ...
                                    if (prTransactionCodes."Employer Deduction") or (prTransactionCodes."Include Employer Deduction") then begin
                                        ...
                                        if curTransAmount > 0 then fnUpdateEmployerDeductions(EmployeeRec."No.", prEmployeeTransactions."Transaction Code", 'EMP', TGroupOrder, TSubGroupOrder, '', curTransAmount, 0, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod);
                                        fnUpdatePeriodTrans(EmployeeRec."No.", 'PENSNORM', TGroup, TGroupOrder, TSubGroupOrder, 'Pension ER', curTransAmount, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, EmployeeRec."Department Code", '', 0, JournalPostingType, prEmployeeTransactions."Loan Number", prTransactionCodes."coop parameters");
                                        fnUpdatePeriodTrans(EmployeeRec."No.", 'PENSNOR', TGroup, TGroupOrder, TSubGroupOrder, 'Pension EE', EERelief, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, EmployeeRec."Department Code", '', 0, JournalPostingType, prEmployeeTransactions."Loan Number", prTransactionCodes."coop parameters");
                                    end;
                                    //Employer deductions
                                end;
                            until prEmployeeTransactions.Next = 0;
                            ...
                            TSubGroupOrder := 9;
                            NoOfUnits := prEmployeeTransactions."No of Units";
                            fnUpdatePeriodTrans(EmployeeRec."No.", strTransCode, TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, TotDed, curTransBalance, intMonth, intYear, prEmployeeTransactions.Membership, prEmployeeTransactions."Reference No", SelectedPeriod, EmployeeRec."Department Code", '', JournalPostAs::" ", JournalPostingType::" ", '', CoopParameters::none);
                            EmployerAmount := 0; //PKK
                            EmployerBalance := 0; //PKK
                                                  //END GET TOTAL DEDUCTIONS
                        end;
                        ...
                        TSubGroupOrder := 0;
                        NoOfUnits := 0;
                        fnUpdatePeriodTrans(EmployeeRec."No.", 'NPAY', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, EmployeeRec."Department Code", PayablesAcc, JournalPostAs::Credit, JournalPostingType::"G/L Account", '', CoopParameters::none);
                        //Rounding Effect: if the Net pay is rounded, take the rounding effect &
                        //save it as an earning for the staff for the next month
                        //>Insert the Netpay rounding effect into the tblRoundingEffect table
                        //Negative pay: if the NetPay<0 then log the entry
