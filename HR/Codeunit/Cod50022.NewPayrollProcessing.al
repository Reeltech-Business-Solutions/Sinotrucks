codeunit 50022 NewPayrollProcessing
{
    // ++Note
    ...
    begin
        ...
        */
        ...
        if curBasicPay > 0 then begin
            ...
            if prEmployeeTransactions.Find('-') then begin
                ...
                repeat
                    ...
                    if prTransactionCodes.Find('-') then begin
                        ...
                        //RESTORED to C/AL: prorate leave allowance for part-year employees
                        ...
                        //RESTORED to C/AL: Bonus Special / Overtime get their own sub-group order
            ...
            curGrossPay := 0;
            //RESTORED to C/AL: posted Gross Pay is net of curNonTaxable
            curGrossPay := (curBasicPay + curTotAllowances + curSalaryArrears) - curNonTaxable;
            ...
            */
            //RESTORED to C/AL (was blocked out to introduce the monthly relief model).
            //NOTE: despite the name, MonthlyReliefAmount holds an ANNUAL figure here -
            //the /12 below is deliberately left commented, exactly as in the C/AL.
            //Annual Relief
            ...
            if ExcludeNonTaxRelief = true then begin
                ...
                if (((((curGrossPay - (currAnnualPay + curNonTaxable)) * 12) + currAnnualPay) * 0.01) +
                   ...
                   (curReliefPersonal + ((curReliefGrossPerc / 100) * (curGrossPay * 12))) then
                    ...
                    MonthlyReliefAmount := curReliefPersonal + ((curReliefGrossPerc / 100) *
                                           ((((curGrossPay - currAnnualPay) * 12) + currAnnualPay)));
            end;
            //Monthly Relief
            //MonthlyReliefAmount:=MonthlyReliefAmount/12;
            //PKK NG
  ...
  //PKK
            ...
            /* //RBS-BANJI Calc AVC amount
              ...
              IF prTransactionCodes.FIND('-') THEN BEGIN
                ...
                IF prTransactionCodes."Is Formula" THEN BEGIN
                  ...
                  END;
              */
            //REMOVED: monthly-relief model that overwrote MonthlyReliefAmount and
            //discarded the NHF and child/dependant relief accumulated above.
            ...
            //RESTORED to C/AL: relief is NOT deducted here - it is applied at the
            //fnGetEmployeePaye call below. Deducting it in both places double-counts it.
            if (curPensionStaff + curDefinedContrib) > curMaxPensionContrib then
                curTaxablePay := curGrossTaxable - (curSalaryArrears + curMaxPensionContrib + curOOI + curHOSP + curNonTaxable + NonTaxable)
            else
                curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curPensionStaff + curOOI + curHOSP + curNonTaxable + NonTaxable);
            ...
            */
            ...
            //PKK
            //RBS-BANJI
            //RESTORED to C/AL: annualise -> charge on annual PAYE bands -> back to monthly.
            //prPAYE."PAYE Tier" holds ANNUAL bands; passing a monthly figure puts almost
            //every employee in the first band.
            statTaxPay := curTaxablePay;
            curTaxablePay := ((curTaxablePay - currAnnualPay) * 12) + currAnnualPay;

            curTaxCharged := fnGetEmployeePaye(curTaxablePay - (MonthlyReliefAmount));
 ...
 begin
                            ...
                            //RESTORED to C/AL: Advance category gets its own sub-group order
            //    //           if (prTransactionCodes."Transaction Category" = prTransactionCodes."Transaction Category"::Advance) then begin
            //                     TGroup := 'DEDUCTIONS';
            //                     TGroupOrder := 8;
            //                     TSubGroupOrder := 5;
            //                 end else begin
                        ...
                        if (prTransactionCodes."Employer Deduction") or (prTransactionCodes."Include Employer Deduction") then begin
                            ...
                            //REMOVED: 'PENSNORM'/'PENSNOR' postings had no C/AL counterpart and
                            //depended on EERelief, which the removed relief model computed.
                            //Re-add together with that formula if pension reporting needs them.
                        end;
                        //Employer deductions
                    //end;
                until prEmployeeTransactions.Next = 0;
            ...
            //RESTORED to C/AL: net pay is net of curNonTaxable
            curNetPay := curGrossPay - (curNSSF + curNHIF + curPAYE + curPayeArrears + curTotalDeductions + IsCashBenefit) - curNonTaxable;
    ...
    begin
        ...
        with prPeriodTransactions do begin
            ...
            if prSalCard.Get(EmpCode) then begin
                ...
                "Location/Division" := prSalCard."Location/Division Code";  //RESTORED to C/AL (was prSalCard.Office)
              ...
              //  "Shortcut Dimension 3 Code" := prSalCard."Shortcut Dimension 3 Code";  //RESTORED to C/AL
            ...
            //REMOVED: "EE Contribution"/"ER Contribution" writes had no C/AL counterpart
            //and depended on EERelief. Re-add with that formula if they are needed.
            Insert;
 ...
 begin
       ...
       // prAssignEmployeeLoan.SetRange(prAssignEmployeeLoan.Status, prAssignEmployeeLoan.Status::Posted);
        ...
        //RESTORED to C/AL: seed "Days Worked" on every active employee's salary card for
        //the new period. Without this, fnBasicPayProrated prorates against stale days.
        EmpRec.Reset;
        EmpRec.SetRange(EmpRec.Status, EmpRec.Status::Active);
 ...
 begin
    ...
    // REMOVED: dead duplicates CalcMonthlyRelief() and NewfnProcesspayroll().

    //RESTORED from C/AL. In NAV this ran from a table trigger on prSalary Card.
    //AL needs it wired as an event subscriber - uncomment the attribute below once you
    //have confirmed which event(s) the original trigger fired on (Insert, Modify, or both).
    ...
    //NOT PORTED: updateSalDetails (C/AL lines 2301-2638, 338 lines) - table trigger on
    //  "HR Employees". Needs an event subscriber and field-by-field verification.
    //NOT PORTED: UpdateBayPayAfterProbation (C/AL 2705-2737) - its entire body is
    //  commented out in the C/AL, so it is a no-op there. Nothing to reproduce.

    //RESTORED from C/AL. Note this is NOT the same as fnDaysInMonth: this one always
    //excludes Sat/Sun, whereas fnDaysInMonth honours VitalSetup."Prol. Based on days in month".
    ...
    //RESTORED from C/AL: prorates a leave allowance for an employee who joined part-way
