codeunit 54509 "HR Dates"
{
    ...
    begin
        if((StartDate <> 0D) and (EndDate <> 0D))then begin
            ...
            if(AbsencePreferences.Comment = true)then WeekendDays:=DetermineWeekends(StartDate, EndDate);
            Holidays:=0;
            AbsencePreferences.Find('-');
            if(AbsencePreferences.Comment = true)then Holidays:=DetermineHolidays(StartDate, EndDate);
