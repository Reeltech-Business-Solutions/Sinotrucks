report 54633 "General Journal - Test2"
{
    // DefaultLayout = RDLC;
    ...
    //                                     if "Gen. Posting Type" = 0 then AddError(StrSubstNo(Text002, FieldCaption("Gen. Posting Type")));
    //                                 end;
    ...
    //                                 if "Gen. Posting Type" <> 0 then AddError(StrSubstNo(Text004, FieldCaption("Gen. Posting Type"), FieldCaption("Account Type"), "Account Type"));
    ...
    //                                     if "Bal. Gen. Posting Type" = 0 then AddError(StrSubstNo(Text002, FieldCaption("Bal. Gen. Posting Type")));
    //                                 end;
    ...
    //             if "Recurring Method" = 0 then AddError(StrSubstNo(Text002, FieldCaption("Recurring Method")));
    ...
    //         else
    //         begin
    //             if "Recurring Method" <> 0 then AddError(StrSubstNo(Text009, FieldCaption("Recurring Method")));
    ...
    //             "Document No.":=DelChr(PadStr(StrSubstNo("Document No.", Day, Week, Month, MonthText, AccountingPeriod.Name), MaxStrLen("Document No.")), '>');
    ...
    //                 if(("Account Type" in["Account Type"::Customer, "Account Type"::Vendor]) and ("Account No." <> '')) or (("Bal. Account Type" in["Bal. Account Type"::Customer, "Bal. Account Type"::Vendor]) and ("Bal. Account No." <> ''))then CurrentCustomerVendors:=CurrentCustomerVendors + 1;
    //                 if(CurrentCustomerVendors > 1) and VATEntryCreated then AddError(StrSubstNo(Text024, "Document Type", "Document No.", "Posting Date"));
    //             end;
    //         end;
    ...
    //         begin
    //             AccName:=Cust.Name;
    //             if((Cust.Blocked = Cust.Blocked::All) or ((Cust.Blocked in[Cust.Blocked::Invoice, Cust.Blocked::Ship]) and ("Document Type" in["Document Type"::Invoice, "Document Type"::" "])))then AddError(StrSubstNo(Text065, "Account Type", Cust.Blocked, FieldCaption("Document Type"), "Document Type"));
    ...
    //             CustPosting:=true;
    //             TestPostingType;
    //             if "Recurring Method" = 0 then if "Document Type" in["Document Type"::Invoice, "Document Type"::"Credit Memo", "Document Type"::"Finance Charge Memo", "Document Type"::Reminder]then begin
    ...
    //             if "Currency Code" <> '' then if not Currency.Get("Currency Code")then AddError(StrSubstNo(Text038, "Currency Code"));
    //             if "Bank Payment Type" <> 0 then if("Bank Payment Type" = "Bank Payment Type"::"Computer Check") and (Amount < 0)then if BankAcc."Currency Code" <> "Currency Code" then AddError(StrSubstNo(Text042, FieldCaption("Bank Payment Type"), FieldCaption("Currency Code"), TableCaption, BankAcc.TableCaption));
    ...
    //             if not("FA Posting Date" in[00020101D .. 99981231D])then AddError(StrSubstNo(Text053, FieldCaption("FA Posting Date")));
    ...
    //             if "Recurring Method" > 0 then AddError(StrSubstNo(TempErrorText, FieldCaption("Recurring Method")));
    ...
    //                         if "IC Partner G/L Acc. No." = '' then AddError(StrSubstNo(Text002, FieldCaption("IC Partner G/L Acc. No.")))
    //                         else
    //                         begin
    //                             if "IC G/L Account".Get("IC Partner G/L Acc. No.")then if "IC G/L Account".Blocked then AddError(StrSubstNo(Text032, "IC G/L Account".FieldCaption(Blocked), false, FieldCaption("IC Partner G/L Acc. No."), "IC Partner G/L Acc. No."));
    //                         end;
    //                     end
    //                     else if "IC Partner G/L Acc. No." <> '' then AddError(StrSubstNo(Text009, FieldCaption("IC Partner G/L Acc. No.")));
    //                 end;
    //             end
    //             else if "IC Partner G/L Acc. No." <> '' then begin
    //                     if "IC Direction" = "IC Direction"::Incoming then AddError(StrSubstNo(Text069, FieldCaption("IC Partner G/L Acc. No."), FieldCaption("IC Direction"), Format("IC Direction")));
    //                     if CurrentICPartner = '' then AddError(StrSubstNo(Text070, FieldCaption("IC Partner G/L Acc. No.")));
    //                 end;
    //         end;
    // end;
    // [Scope('Cloud')]
