report 54721 "prPaySlip2 Taxable1-Self serv"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("HR-Employee"; "HR-Employee")
        {
            ...
            dataitem("prSalary Card"; "prSalary Card")
            {
                ...
                begin
                    ...
                    if objEmp.Find('-')then begin
                        ...
                        if objEmp."Date Of Leaving" = 0D then dtOfLeaving:=DMY2Date(31, 12, 9999)
                        else
                    ...
                    *****************************************************************************************************/
                    ...
                    if PeriodTrans.Find('-')then repeat TaxPayTillDate:=TaxPayTillDate + PeriodTrans.Amount;
                    ...
                    Index:=1;
                    strGrpText:='';
                    if PeriodTrans.Find('-')then repeat if(PeriodTrans."Transaction Code" <> 'TXCHRG') and (PeriodTrans."Transaction Code" <> 'NONTAX') and (PeriodTrans."Transaction Code" <> 'PENS')then begin //PKK  NONTAX
                                if strGrpText <> PeriodTrans."Group Text" then begin
                                    ...
                                    if PeriodTrans."Transaction Code" = 'TOT-DED' then Evaluate(TransAmt[ColumnNo, Index], Format(Round(PeriodTrans.Amount, 0.01)))
                                    //Message('%1',EVALUATE(TransAmt[ColumnNo,Index],FORMAT(ROUND(PeriodTrans.Amount+StatutoryAmt,0.01))));
                                    else if PeriodTrans."Group Order" = 4 then Evaluate(TransAmt[ColumnNo, Index], Format(Round(PeriodTrans.Amount - NonTaxable, 0.01)))
                                        else if PeriodTrans."Group Order" = 9 then Evaluate(TransAmt[ColumnNo, Index], Format(Round(PeriodTrans.Amount - NonTaxable, 0.01)))
                                            else if PeriodTrans."Transaction Code" = 'NONTAX' then Evaluate(TransAmt[ColumnNo, Index], Format(Round(PeriodTrans.Amount - NonTaxable, 0.01)))
                                                else
            ...
            begin
                ...
                //Message(
                if UserId <> hrEmp."User ID" then CurrReport.Quit;
            end;
        }
    }
    requestpage
