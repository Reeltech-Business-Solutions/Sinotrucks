report 54718 "prPaySlip2 Taxable1"
{
    DefaultLayout = RDLC;
    RDLCLayout = './prPaySlip2TaxableNew.rdl';
    ApplicationArea = All;
        ...
        dataitem("HR-Employee"; "HR-Employee")
        {
            ...
            dataitem("prSalary Card"; "prSalary Card")
            {
                ...
                begin
                    ...
                    *****************************************************************************************************/
                    ...
                    if PeriodTrans.Find('-') then
                        repeat
                            if (PeriodTrans."Transaction Code" <> 'TXCHRG') and (PeriodTrans."Transaction Code" <> 'NONTAX') and (PeriodTrans."Transaction Code" <> 'PENS') then begin //PKK  NONTAX
            ...
            begin
                ...
                if EmpNoFilter <> '' then begin
                    SetFilter("No.", '%1', EmpNoFilter);
                    hrEmp.Get(EmpNoFilter);
                end else
                    hrEmp.Get("HR-Employee".GetFilter("No."));
                // UserSetup.Get(UserId);
                // if UserSetup."PayRoll Cent. Filter" <> hrEmp."Payroll Filter Group" then CurrReport.Quit;
            end;
        }
    }
    requestpage
    ...
    begin
        ...
        if VPeriodFilter = 0D then begin
            ...
            //PeriodFilter := "HR-Employee".GetFilter("Current Month Filter"); //"prSalary Card".GETFILTER("Period Filter");
            PeriodFilter := Format(VPeriodFilter);

            //SelectedPeriod := "HR-Employee".GetRangeMin("Current Month Filter"); //"prSalary Card".GETRANGEMIN("Period Filter");
            SelectedPeriod := VPeriodFilter;
            objPeriod.Reset;
            if objPeriod.Get(SelectedPeriod) then PeriodName := objPeriod."Period Name";
        end;
    ...
    procedure filterForDate(PeriodDate: Date; employeeNo: code[20]; frominteg: Boolean)
    begin
        EmpNoFilter := employeeNo;
        fromIntegration := frominteg;
        VPeriodFilter := PeriodDate;
        //VPeriodFilter := 20230501D;
    end;
