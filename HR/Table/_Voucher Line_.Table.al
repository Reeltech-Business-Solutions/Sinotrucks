...
//             trigger OnValidate()
//             begin
//                 IF VoucherHeader.GET("Voucher Type", "Document No.")THEN VALIDATE("Responsibility Center", VoucherHeader."Responsibility Center");
//             end;
//         }
...
//             ELSE IF(Account=CONST(Customer), "Responsibility Center"=FILTER(<>''))Customer."No." WHERE(Blocked=FILTER(' '), "Responsibility Center"=FIELD("Responsibility Center"), "Customer Type"=filter(<>Employee))
//             ELSE IF(Account=CONST(Customer), "Responsibility Center"=FILTER(''))Customer."No." WHERE(Blocked=FILTER(' '), "Customer Type"=filter(<>Employee))
//             ELSE IF(Account=CONST("Local Vendor"))Vendor."No." WHERE(Blocked=FILTER(' '), "Vendor Type"=FILTER(Local))
//             ELSE IF(Account=FILTER("Foreign Vendor"))Vendor."No." WHERE(Blocked=FILTER(' '), "Vendor Type"=FILTER(Import))
//             ELSE if(Account=Const("Local Staff"))vendor."No." where(Blocked=FIlter(' '), "Vendor Type"=filter(Cash))
//             else IF(Account=FILTER(Cash))"Bank Account"."No." WHERE(Blocked=CONST(false), Cash=CONST(true))
//             ELSE IF(Account=FILTER(Bank))"Bank Account"."No." WHERE(Blocked=CONST(false))
//             ELSE IF(Account=FILTER("Fixed Asset"))"Fixed Asset"."No." WHERE(Blocked=FILTER(false))
//             else if(Account=filter(Employee))Employee."No." Where(Status=filter(Active));
...
//                 END;
//                 END;
//                 CreateDim(DimMgt.TypeToTableID1("Account Type"), "Account No.", DimMgt.TypeToTableID1("Bal. Account Type"), "Bal. Account No.", DATABASE::Job, "Job No.", DATABASE::"Salesperson/Purchaser", "Salespers./Purch. Code", DATABASE::Campaign, "Campaign No.");
//             end;
//         }
...
//                         GetCurrency;
//                         Amount:=ROUND(CurrExchRate.ExchangeAmtLCYToFCY("Posting Date", "Currency Code", "Amount (LCY)", "Currency Factor"), Currency."Amount Rounding Precision")END
//                     ELSE
//                     BEGIN
...
//                 GetCurrency;
//                 IF "Currency Code" <> '' THEN BEGIN
//                     IF("Currency Code" <> xRec."Currency Code") OR ("Posting Date" <> xRec."Posting Date") OR ("Currency Factor" = 0)THEN "Currency Factor":=CurrExchRate.ExchangeRate("Posting Date", "Currency Code");
//                 END
//                 ELSE
//                     "Currency Factor":=0;
//                 VALIDATE("Currency Factor");
//                 IF "Currency Factor" <> 0 THEN VALIDATE("Exchange Rate", (1 / "Currency Factor"));
//                 IF "Currency Code" = '' THEN "Exchange Rate":=0;
//             end;
//         }
...
//                 IF "Exchange Rate" <> 0 THEN VALIDATE("Currency Factor", (1 / "Exchange Rate"));
//             end;
//         }
...
//         {
//             OptionCaption = 'G/L Account,Customer,Local Vendor,Foreign Vendor,Cash,Bank,Fixed Asset,Employee';
//             OptionMembers = "G/L Account", Customer, "Local Vendor", "Foreign Vendor", "Local Staff", "Import File", Cash, Bank, "Fixed Asset", Employee;

//             trigger OnValidate()
//             begin
//                 TestStatusOpen;
//                 CASE Account OF Account::"G/L Account": VALIDATE("Account Type", Account::"G/L Account");
//                 Account::Customer: VALIDATE("Account Type", Account::Customer);
//                 Account::"Local Vendor": VALIDATE("Account Type", Account::"Local Vendor");
//                 Account::"Foreign Vendor": VALIDATE("Account Type", Account::"Foreign Vendor");
//                 /*Account::Intercomany:
//                         VALIDATE("Account Type", "Account Type"::Vendor);
//                     Account::"Expat Staff":
//                         VALIDATE("Account Type", "Account Type"::Vendor);
//                         */
//                 Account::"Local Staff": VALIDATE("Account Type", "Account Type"::Vendor);
//                 Account::"Import File": VALIDATE("Account Type", "Account Type"::Vendor);
//                 Account::Cash: VALIDATE("Account Type", "Account Type"::"Bank Account");
//                 Account::Bank: VALIDATE("Account Type", "Account Type"::"Bank Account");
//                 /*
//                 Account::"Other Bank":
//                     VALIDATE("Account Type", "Account Type"::"Bank Account");
//                     */
//                 Account::"Fixed Asset": VALIDATE("Account Type", "Account Type"::"Fixed Asset");
//                 Account::Employee: VALIDATE("Account Type", "Account Type"::Employee);
//                 END;
//             end;
//         }
...
//             ELSE IF("Bal. Account Type"=CONST(Customer), "Responsibility Center"=FILTER(<>''))Customer."No." WHERE(Blocked=FILTER(' '), "Responsibility Center"=FIELD("Responsibility Center"))
//             ELSE IF("Bal. Account Type"=CONST(Customer), "Responsibility Center"=FILTER(''))Customer."No." WHERE(Blocked=FILTER(' '))
//             ELSE IF("Bal. Account Type"=CONST(Vendor))Vendor."No." WHERE(Blocked=FILTER(' '), "Vendor Type"=FILTER(Local))
//             ELSE IF("Bal. Account Type"=FILTER("Bank Account"))"Bank Account"."No." WHERE(Blocked=CONST(false))
//             ELSE IF("Bal. Account Type"=FILTER("Fixed Asset"))"Fixed Asset"."No." WHERE(Blocked=FILTER(false))
//             else if("Bal. Account Type"=filter(Employee))Employee."No." Where(Status=filter(Active));
...
//                 END;
//                 //UNL <<
//                 CASE "Bal. Account Type" OF "Bal. Account Type"::"G/L Account": BEGIN
...
//                     VALIDATE("Currency Code");
//                 END;
//                 "Bal. Account Type"::Employee: BEGIN
...
//                     VALIDATE("Currency Code");
//                 END;
//                 "Bal. Account Type"::"Fixed Asset": BEGIN
//                     FA.GET("Bal. Account No.");
//                     "Account Name":=FA.Description;
//                 END;
//                 END;
//                 CreateDim(DimMgt.TypeToTableID1("Bal. Account Type"), "Bal. Account No.", DimMgt.TypeToTableID1("Bal. Account Type"), "Bal. Account No.", DATABASE::Job, "Job No.", DATABASE::"Salesperson/Purchaser", "Salespers./Purch. Code", DATABASE::Campaign, "Campaign No.");
//             end;
//         }
...
//                 CASE Account OF Account::Customer: BEGIN
...
//                     END;
//                 END;
//                 Account::"Local Vendor", Account::"Foreign Vendor": BEGIN
...
//                     IF NOT(xRec."Applies-to Doc. Type" = "Applies-to Doc. Type"::" ")THEN CustLedgEntry.SETRANGE("Document Type", xRec."Applies-to Doc. Type");
...
//                     END;
//                 END
//                 ELSE IF TempVoucherLine."Account Type" = TempVoucherLine."Account Type"::Employee THEN BEGIN
//                         EmpLedgEntry.SETCURRENTKEY("Document No.");
//                         EmpLedgEntry.SETRANGE("Document No.", xRec."Applies-to Doc. No.");
//                         IF NOT(xRec."Applies-to Doc. Type" = "Applies-to Doc. Type"::" ")THEN EmpLedgEntry.SETRANGE("Document Type", xRec."Applies-to Doc. Type");
...
//                     //END;
//                     END
//                     ELSE IF TempVoucherLine."Account Type" = TempVoucherLine."Account Type"::Vendor THEN BEGIN
//                             VendLedgEntry.SETCURRENTKEY("Document No.");
//                             VendLedgEntry.SETRANGE("Document No.", xRec."Applies-to Doc. No.");
//                             IF NOT(xRec."Applies-to Doc. Type" = "Applies-to Doc. Type"::" ")THEN VendLedgEntry.SETRANGE("Document Type", xRec."Applies-to Doc. Type");
...
//             trigger OnValidate()
//             begin
//                 IF("Bank Payment Type" <> "Bank Payment Type"::" ") AND ("Account Type" <> "Account Type"::"Bank Account")THEN ERROR(Text007, FIELDCAPTION("Account Type"));
//                 IF("Account Type" = "Account Type"::"Fixed Asset")THEN FIELDERROR("Account Type");
//             end;
//         }
...
//             trigger OnValidate()
//             begin
//                 IF NOT(("Account Type" = "Account Type"::"Fixed Asset") OR ("Bal. Account Type" = "Bal. Account Type"::"Fixed Asset")) AND ("FA Posting Type" = "FA Posting Type"::" ")THEN BEGIN
//                     "FA Posting Date":=0D;
//                     "Salvage Value":=0;
...
//                 IF "FA Posting Type" <> "FA Posting Type"::"Acquisition Cost" THEN TESTFIELD("Insurance No.", '');
//                 IF "FA Posting Type" <> "FA Posting Type"::Maintenance THEN TESTFIELD("Maintenance Code", '');
//             /*GetFAVATSetup;
...
//             end;
//         }
//         field(50008; "Responsibility Center"; Code[10])
//         {
...
//             trigger OnValidate()
//             begin
//                 CreateDim(DATABASE::Campaign, "Campaign No.", DimMgt.TypeToTableID1("Account Type"), "Account No.", DimMgt.TypeToTableID1("Bal. Account Type"), "Bal. Account No.", DATABASE::Job, "Job No.", DATABASE::"Salesperson/Purchaser", "Salespers./Purch. Code");
//             end;
//         }
...
//             trigger OnValidate()
//             begin
//                 IF("Applies-to ID" <> xRec."Applies-to ID") AND (xRec."Applies-to ID" <> '')THEN ClearCustVendApplnEntry;
...
//         }
//         field(50017; "Maintenance Code"; Code[10])
//         {
...
//             end;
//         }
//         field(50020; "Duplicate in Depreciation Book"; Code[10])
//         {
...
//         }
//         field(50024; "Depreciation Book Code"; Integer)
//         {
//             BlankZero = true;
//             Caption = 'FA Error Entry No.';
//             TableRelation = "Depreciation Book";
...
//         {
//             CalcFormula = Lookup("Dimension Value".Name WHERE("Dimension Code"=FILTER('EXPATS'), Code=FIELD("Shortcut Dimension 7 Code")));
//             Editable = false;
...
//             Editable = false;
//         }
//         field(60012; "Salvage Value"; Integer)
//         {
//             Editable = false;
//         }
//         field(60013; "Exported to Payment File"; Boolean)
//         {
//         }
//     }
//     keys
...
//             CLEAR(Currency);
//             Currency.InitRoundingPrecision END
//         ELSE IF CurrencyCode <> Currency.Code THEN BEGIN
//                 Currency.GET(CurrencyCode);
//                 Currency.TESTFIELD("Amount Rounding Precision");
//             END;
//     end;
//     [Scope('Cloud')]
//     procedure CheckFixedCurrency(): Boolean var
//         CurrExchRate: Record "Currency Exchange Rate";
//     begin
//         CurrExchRate.SETRANGE("Currency Code", "Currency Code");
//         CurrExchRate.SETRANGE("Starting Date", 0D, "Posting Date");
//         IF NOT CurrExchRate.FIND('+')THEN EXIT(FALSE);
//         IF CurrExchRate."Relational Currency Code" = '' THEN EXIT(CurrExchRate."Fix Exchange Rate Amount" = CurrExchRate."Fix Exchange Rate Amount"::Both);
//         IF CurrExchRate."Fix Exchange Rate Amount" <> CurrExchRate."Fix Exchange Rate Amount"::Both THEN EXIT(FALSE);
//         CurrExchRate.SETRANGE("Currency Code", CurrExchRate."Relational Currency Code");
//         IF CurrExchRate.FIND('+')THEN EXIT(CurrExchRate."Fix Exchange Rate Amount" = CurrExchRate."Fix Exchange Rate Amount"::Both);
//         EXIT(FALSE);
//     end;
//     [Scope('Cloud')]
...
//     begin
//         "Dimension Set ID":=DimMgt.EditDimensionSet("Dimension Set ID", STRSUBSTNO('%1 %2 %3', "Voucher Type", "Document No.", "Line No."));
...
//     begin
//         IF VoucherHeader.GET("Voucher Type", "Document No.")THEN VoucherHeader.TESTFIELD(Status, VoucherHeader.Status::Open);
//     end;
//     [Scope('Cloud')]
...
//             CustLedgEntry.SETRANGE("Customer No.", "Account No.");
//             CustLedgEntry.SETRANGE(Open, TRUE);
//             IF CustLedgEntry.FIND('-')THEN IF CustLedgEntry."Amount to Apply" = 0 THEN BEGIN
//                     CustLedgEntry.CALCFIELDS("Remaining Amount");
//                     CustLedgEntry."Amount to Apply":=CustLedgEntry."Remaining Amount";
//                     CODEUNIT.RUN(CODEUNIT::"Cust. Entry-Edit", CustLedgEntry);
//                 END;
//         END
//         ELSE IF Account = Account::Employee THEN BEGIN
//                 EmpLedgEntry.SETCURRENTKEY("Document No.");
//                 EmpLedgEntry.SETRANGE("Document No.", Rec."Applies-to Doc. No.");
//                 EmpLedgEntry.SETRANGE("Employee No.", "Account No.");
//                 EmpLedgEntry.SETRANGE(Open, TRUE);
//                 IF EmpLedgEntry.FIND('-')THEN IF EmpLedgEntry."Amount to Apply" = 0 THEN BEGIN
//                         EmpLedgEntry.CALCFIELDS("Remaining Amount");
//                         EmpLedgEntry."Amount to Apply":=EmpLedgEntry."Remaining Amount";
//                         CODEUNIT.RUN(CODEUNIT::"Empl. Entry-Edit", EmpLedgEntry);
//                     END;
//             END
//             ELSE IF(Account = Account::"Local Vendor") OR ((Account = Account::"Foreign Vendor"))THEN BEGIN
//                     VendLedgEntry.SETCURRENTKEY("Document No.");
//                     VendLedgEntry.SETRANGE("Document No.", Rec."Applies-to Doc. No.");
//                     VendLedgEntry.SETRANGE("Vendor No.", "Account No.");
//                     VendLedgEntry.SETRANGE(Open, TRUE);
//                     IF VendLedgEntry.FIND('-')THEN IF VendLedgEntry."Amount to Apply" = 0 THEN BEGIN
...
//                 IF TempVoucherLine."Applies-to Doc. Type" <> TempVoucherLine."Applies-to Doc. Type"::" " THEN CustLedgEntry.SETRANGE("Document Type", TempVoucherLine."Applies-to Doc. Type");
//                 CustLedgEntry.SETRANGE("Customer No.", TempVoucherLine."Account No.");
//                 CustLedgEntry.SETRANGE(Open, TRUE);
//                 IF CustLedgEntry.FIND('-')THEN IF(TempVoucherLine."Posting Date" < CustLedgEntry."Posting Date")THEN ERROR(Text008, TempVoucherLine."Voucher Type", TempVoucherLine."Document No.", CustLedgEntry."Document Type", CustLedgEntry."Document No.");
//             END;
//             IF TempVoucherLine."Account Type" = TempVoucherLine."Account Type"::Vendor THEN BEGIN
//             END
//             ELSE IF TempVoucherLine."Applies-to Doc. No." <> '' THEN BEGIN
//                     VendLedgEntry.SETCURRENTKEY("Document No.");
//                     VendLedgEntry.SETRANGE("Document No.", TempVoucherLine."Applies-to Doc. No.");
//                     IF TempVoucherLine."Applies-to Doc. Type" <> TempVoucherLine."Applies-to Doc. Type"::" " THEN VendLedgEntry.SETRANGE("Document Type", TempVoucherLine."Applies-to Doc. Type");
//                     VendLedgEntry.SETRANGE("Vendor No.", TempVoucherLine."Account No.");
//                     VendLedgEntry.SETRANGE(Open, TRUE);
//                     IF VendLedgEntry.FIND('-')THEN IF(TempVoucherLine."Posting Date" < VendLedgEntry."Posting Date")THEN ERROR(Text008, TempVoucherLine."Voucher Type", TempVoucherLine."Document No.", VendLedgEntry."Document Type", VendLedgEntry."Document No.");
//                 END;
...
//                     IF TempVoucherLine."Applies-to Doc. Type" <> TempVoucherLine."Applies-to Doc. Type"::" " THEN EmpLedgEntry.SETRANGE("Document Type", TempVoucherLine."Applies-to Doc. Type");
//                     EmpLedgEntry.SETRANGE("Employee No.", TempVoucherLine."Account No.");
//                     EmpLedgEntry.SETRANGE(Open, TRUE);
//                     IF EmpLedgEntry.FIND('-')THEN IF(TempVoucherLine."Posting Date" < EmpLedgEntry."Posting Date")THEN ERROR(Text008, TempVoucherLine."Voucher Type", TempVoucherLine."Document No.", EmpLedgEntry."Document Type", EmpLedgEntry."Document No.");
//                 END;
//         END;
//     end;
//     [Scope('Cloud')]
...
//     // For example, you can call other methods or perform some operations
//     // If you don't need any specific implementation, you can leave it empty
//     end;
//     [Scope('Cloud')]
//     procedure CheckGLAcc()
//     begin
//         GLAcc.CheckGLAcc;
//         IF GLAcc."Direct Posting" THEN EXIT;
//         IF "Posting Date" <> 0D THEN IF "Posting Date" = CLOSINGDATE("Posting Date")THEN EXIT;
...
//         CASE AccType OF AccType::Customer: IF "Applies-to ID" <> '' THEN BEGIN
//                 IF FindFirstCustLedgEntryWithAppliesToID(AccNo)THEN BEGIN
//                     ClearCustApplnEntryFields;
//                     CustEntrySetApplID.SetApplId(CustLedgEntry, TempCustLedgEntry, '');
//                 END END
//             ELSE IF "Applies-to Doc. No." <> '' THEN IF FindFirstCustLedgEntryWithAppliesToDocNo(AccNo)THEN BEGIN
//                         ClearCustApplnEntryFields;
//                         CustEntryEdit.RUN(CustLedgEntry);
//                     END;
//         AccType::Vendor: IF "Applies-to ID" <> '' THEN BEGIN
//                 IF FindFirstVendLedgEntryWithAppliesToID(AccNo)THEN BEGIN
//                     ClearVendApplnEntryFields;
//                     VendEntrySetApplID.SetApplId(VendLedgEntry, TempVendLedgEntry, '');
//                 END END
//             ELSE IF "Applies-to Doc. No." <> '' THEN IF FindFirstVendLedgEntryWithAppliesToDocNo(AccNo)THEN BEGIN
//                         ClearVendApplnEntryFields;
...
//         END;
//     end;
//     local procedure FindFirstCustLedgEntryWithAppliesToID(AccNo: Code[20]): Boolean begin
//         CustLedgEntry.RESET;
//         CustLedgEntry.SETCURRENTKEY("Customer No.", "Applies-to ID", Open);
//         CustLedgEntry.SETRANGE("Customer No.", AccNo);
//         CustLedgEntry.SETRANGE("Applies-to ID", "Applies-to ID");
//         CustLedgEntry.SETRANGE(Open, TRUE);
//         EXIT(CustLedgEntry.FINDFIRST)end;
//     local procedure FindFirstEmpLedgEntryWithAppliesToID(AccNo: Code[20]): Boolean begin
//         EmpLedgEntry.RESET;
...
//         EmpLedgEntry.SETRANGE(Open, TRUE);
//         EXIT(EmpLedgEntry.FINDFIRST)end;
//     local procedure FindFirstVendLedgEntryWithAppliesToID(AccNo: Code[20]): Boolean begin
//         VendLedgEntry.RESET;
//         VendLedgEntry.SETCURRENTKEY("Vendor No.", "Applies-to ID", Open);
//         VendLedgEntry.SETRANGE("Vendor No.", AccNo);
//         VendLedgEntry.SETRANGE("Applies-to ID", "Applies-to ID");
//         VendLedgEntry.SETRANGE(Open, TRUE);
//         EXIT(VendLedgEntry.FINDFIRST)end;
...
//         VendLedgEntry."Amount to Apply":=0;
//     end;
//     local procedure FindFirstCustLedgEntryWithAppliesToDocNo(AccNo: Code[20]): Boolean begin
//         CustLedgEntry.RESET;
//         CustLedgEntry.SETCURRENTKEY("Document No.");
...
//         CustLedgEntry.SETRANGE(Open, TRUE);
//         EXIT(CustLedgEntry.FINDFIRST)end;
//     local procedure FindFirstEmpLedgEntryWithAppliesToDocNo(AccNo: Code[20]): Boolean begin
//         EmpLedgEntry.RESET;
//         EmpLedgEntry.SETCURRENTKEY("Document No.");
...
//         EmpLedgEntry.SETRANGE(Open, TRUE);
//         EXIT(EmpLedgEntry.FINDFIRST)end;
//     local procedure FindFirstVendLedgEntryWithAppliesToDocNo(AccNo: Code[20]): Boolean begin
//         VendLedgEntry.RESET;
//         VendLedgEntry.SETCURRENTKEY("Document No.");
