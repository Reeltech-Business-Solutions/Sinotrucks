table 50120 "Voucher Line"
{
    ...
    fields
    {
        ...
        field(4; "Account No."; Code[20])
        {
            ...
            ELSE
            IF (Account = CONST(Customer), "Responsibility Center" = FILTER(<> '')) Customer."No." WHERE(Blocked = FILTER(' '), "Responsibility Center" = FIELD("Responsibility Center"), "Customer Type" = filter(<> "HR Employee"))
            ELSE
            IF (Account = CONST(Customer), "Responsibility Center" = FILTER('')) Customer."No." WHERE(Blocked = FILTER(' '), "Customer Type" = filter(<> "HR Employee"))
            ELSE
